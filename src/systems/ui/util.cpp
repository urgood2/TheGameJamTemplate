...
namespace ui
{
    ...
    void util::ApplyTransformMatrixImmediate(const float& visualX,  const float& visualY,  const float& visualW,  const float& visualH,  const float& visualScaleWithHoverAndDynamicMotionReflected,  const float& visualR, const float& rotationOffset, layer::Layer* layerPtr, std::optional<Vector2> addedOffset, bool applyOnlyTranslation)
    {
    ...
    void util::ApplyTransformMatrix(const float& visualX,  const float& visualY,  const float& visualW,  const float& visualH,  const float& visualScaleWithHoverAndDynamicMotionReflected,  const float& visualR, const float& rotationOffset, layer::Layer* layerPtr, std::optional<Vector2> addedOffset, bool applyOnlyTranslation, const int& zIndex)
    {
    ...
    {
        ZONE_SCOPED("ui::util::emplaceOrReplaceNewRectangleCache");
    ...
    {
        ZONE_SCOPED("ui::util::DrawNPatchUIElement");
        ...
        if (uiConfig->shadow)
        {
            ...
            util::ApplyTransformMatrix(visualX, visualY, visualW, visualH, visualScaleWithHoverAndMotion, visualR, rotationOffset, layerPtr.get(), Vector2{-shadowOffsetX * parallaxModifier, -shadowOffsetY * parallaxModifier}, false, zIndex);
        ...
        util::ApplyTransformMatrix(visualX, visualY, visualW, visualH, visualScaleWithHoverAndMotion, visualR, rotationOffset, layerPtr.get(), Vector2{0, 0}, false, zIndex);
    ...
    void util::DrawNPatchUIElementImmediate(layer::Layer* layerPtr, entt::registry &registry, entt::entity entity, const Color &colorOverride, float parallaxModifier, std::optional<float> progress)
    {
        ZONE_SCOPED("ui::util::DrawNPatchUIElement");
        ::util::Profiler profiler("DrawNPatchUIElement");
        auto &transform = registry.get<transform::Transform>(entity);
    ...
    namespace {
        std::vector<const UIDecoration*> CollectUIDecorations(const UIDecorations& decorations, bool underlay) {
            std::vector<const UIDecoration*> sorted;
            sorted.reserve(decorations.items.size());
            for (const auto& d : decorations.items) {
                if (!d.visible) continue;
                if (underlay) {
                    if (d.zOffset < 0) sorted.push_back(&d);
                } else {
                    if (d.zOffset >= 0) sorted.push_back(&d);
                }
            }
        ...
        void DrawUIDecorationsInternal(layer::Layer* layerPtr, const UIDecorations& decorations,
            float parentX, float parentY, float parentW, float parentH, bool underlay)
        {
            if (decorations.items.empty()) return;

            const auto sorted = CollectUIDecorations(decorations, underlay);
            ...
            for (const auto* decor : sorted) {
                ...
                const float uiScale = globals::getGlobalUIScaleFactor();
                float drawW = frame.frame.width * decor->scale.x * uiScale;
                float drawH = frame.frame.height * decor->scale.y * uiScale;
                float anchorPosX = parentX + anchorX + (decor->offset.x * uiScale);
                float anchorPosY = parentY + anchorY + (decor->offset.y * uiScale);
        ...
        void DrawUIDecorationsQueuedInternal(layer::Layer* layerPtr, const UIDecorations& decorations,
            float parentX, float parentY, float parentW, float parentH, int zIndex, bool underlay)
        {
            if (decorations.items.empty()) return;
    ...
    {
        DrawUIDecorationsInternal(layerPtr, decorations, parentX, parentY, parentW, parentH, false);
    }

    void util::DrawUIDecorationsUnderlay(layer::Layer* layerPtr, const UIDecorations& decorations, float parentX, float parentY, float parentW, float parentH)
    {
        DrawUIDecorationsInternal(layerPtr, decorations, parentX, parentY, parentW, parentH, true);
    }
    ...
    void util::DrawUIDecorationsQueued(layer::Layer* layerPtr, const UIDecorations& decorations, float parentX, float parentY, float parentW, float parentH, int zIndex)
    {
        ...
        DrawUIDecorationsQueuedInternal(layerPtr, decorations, parentX, parentY, parentW, parentH, zIndex, false);
    }

    void util::DrawUIDecorationsUnderlayQueued(layer::Layer* layerPtr, const UIDecorations& decorations, float parentX, float parentY, float parentW, float parentH, int zIndex)
    {
        DrawUIDecorationsQueuedInternal(layerPtr, decorations, parentX, parentY, parentW, parentH, zIndex, true);
    }
    ...
    void util::DrawSteppedRoundedRectangle(std::shared_ptr<layer::Layer> layerPtr, entt::registry &registry, entt::entity entity, transform::Transform &transform, ui::UIConfig* uiConfig, transform::GameObject &node, RoundedRectangleVerticesCache* rectCache, const float &visualX, const float & visualY, const float & visualW, const float & visualH, const float & visualScaleWithHoverAndMotion, const float & visualR, const float & rotationOffset, const int &type, float parallaxModifier, const std::unordered_map<std::string, Color> &colorOverrides, std::optional<float> progress, std::optional<float> lineWidthOverride, const int &zIndex)
    {
        ...
        ZONE_SCOPED("ui::util::DrawSteppedRoundedRectangle");
        ...
        {
            ZONE_SCOPED("ui::util::DrawSteppedRoundedRectangle cache checks");
        ...
        {
            ZONE_SCOPED("ui::util::DrawSteppedRoundedRectangle clip regen");
        ...
        {
            ZONE_SCOPED("ui::util::DrawSteppedRoundedRectangle shadow fill");
        ...
        {
            ZONE_SCOPED("ui::util::DrawSteppedRoundedRectangle shadow outline");
            layer::QueueCommand<layer::CmdPushMatrix>(layerPtr, [](layer::CmdPushMatrix *cmd) {}, zIndex);
        ...
        {
            ZONE_SCOPED("ui::util::DrawSteppedRoundedRectangle emboss fill");
            layer::QueueCommand<layer::CmdPushMatrix>(layerPtr, [](layer::CmdPushMatrix *cmd) {}, zIndex);

            if (!uiConfig->emboss)
                SPDLOG_DEBUG("Emboss value not provided for emboss fill rectangle render flag");
                
            util::ApplyTransformMatrix(visualX, visualY, visualW, visualH, visualScaleWithHoverAndMotion, visualR, rotationOffset, layerPtr.get(), Vector2{0, uiConfig->emboss.value_or(5.f) * parallaxModifier * uiConfig->scale.value_or(1.0f)}, false, zIndex);
        ...
        {
            ZONE_SCOPED("ui::util::DrawSteppedRoundedRectangle emboss outline");
            layer::QueueCommand<layer::CmdPushMatrix>(layerPtr, [](layer::CmdPushMatrix *cmd) {}, zIndex);
...
// 
        ...
        {
            ZONE_SCOPED("ui::util::DrawSteppedRoundedRectangle fill");
        ...
        {
            ZONE_SCOPED("ui::util::DrawSteppedRoundedRectangle progress fill");
            layer::QueueCommand<layer::CmdPushMatrix>(layerPtr, [](layer::CmdPushMatrix *cmd) {}, zIndex);
        ...
        {
            ZONE_SCOPED("ui::util::DrawSteppedRoundedRectangle outline");
            layer::QueueCommand<layer::CmdPushMatrix>(layerPtr, [](layer::CmdPushMatrix *cmd) {}, zIndex);
    ...
    void util::RenderRectVerticesFilledLayerImmediate(layer::Layer* layerPtr, const Rectangle outerRec, const std::vector<Vector2> &outerVertices, const Color color)
    {
