# Serpent — SNKRX-Style Survivor (Vertical Slice) Implementation Plan v5.2
...
- `COMBAT` → `SHOP` when:
  - `wave_director:is_done_spawning()` is true **and**
...
1. Controllers:
   - `snake_controller:update(dt, snake_entities, input)`
   - `enemy_controller:update(dt, enemy_entities, head_pos)`
2. Build snapshots:
   - `segment_snaps = snake_entity_adapter:build_snapshots()`
   - `contact_snaps = contact_collector:build_snapshot()` (sorted)
3. Pure combat:
   - `(snake_state, enemy_snaps, combat_state, events) = combat_logic.tick(dt, ...)`
4. Apply events to runtime:
   - `combat_adapter:apply(events, ...)` (despawn + unregister on death)
5. Spawning (pure scheduling, then runtime spawn):
   - `(director_state, id_state, spawn_events) = wave_director.tick(dt, director_state, id_state, rng, events, alive_enemy_ids_set)`
   - `enemy_spawner_adapter:apply(spawn_events, ...)` (creates runtime entities + extends `enemy_snaps`)
...
- `hp = hp_max`
- `speed = enemy_def.speed` (no scaling)
- `tags = enemy_def.tags`
...
7. Consume `ContactSnapshot` and apply contact cooldown gating per (enemy_id, instance_id), emit `damage_unit` with `amount_int = enemy_snapshot.damage`.
...
10. Feed relevant events into specials logic:
...
- Director maintains:
  ...
  - `delayed_queue` array of `{ t_left_sec, def_id }` for future boss spawns
...
- Per `tick(dt, ...)`:
  1) Decrement delayed timers; move any `t_left_sec <= 0` into `forced_queue` (preserve original order).
  2) Process combat events relevant to bosses (from `events[]`):
  ...
  3) Boss periodic spawns:
     - For each tracked boss state that is alive, call its `tick(...)` to enqueue more spawns:
       - `swarm_queen`: every `10.0s`, append `5` `slime` to `forced_queue` (in-order).
       - `lich_king`: any queued “raise” becomes a delayed entry of `{ t_left_sec=2.0, def_id="skeleton" }`.
...
- Build `base_spawn_list` length `n` with:
  - for `i=1..n`: `base_spawn_list[i] = rng:choice(pool)` (uniform)
- Boss injection:
...
- `assets/scripts/serpent/enemy_factory.lua`:
  - `create_snapshot(enemy_def, enemy_id, wave_num, wave_config) -> EnemySnapshot` (scaled hp/damage + tags)
...
- `assets/scripts/serpent/serpent_wave_director.lua`:
  ...
  - constants:
    ...
    - `MAX_SPAWNS_PER_FRAME = 3` (global cap across base + boss spawns)
  - API:
    - `start_wave(wave_num, rng, enemy_defs) -> director_state`
  ...
  - must implement:
    ...
    - swarm queen cadence and lich raise scheduling exactly as specified
...
- `assets/scripts/serpent/tests/test_serpent_wave_director.lua`:
  ...
  - boss behaviors:
    ...
    - lich: queues 1 `skeleton` spawn 2.0s after each non-boss enemy death while alive
...
- `assets/scripts/serpent/enemy_spawner_adapter.lua`:
  ...
  - uses `enemy_factory.create_snapshot(...)` to create scaled stats
...
- `assets/scripts/serpent/bosses/swarm_queen.lua`
  ...
  - `tick(dt, boss_state, is_alive, rng) -> (boss_state, forced_def_ids[])`
...
- `assets/scripts/serpent/bosses/lich_king.lua`
  ...
  - delayed spawn shape: `{ t_left_sec=2.0, def_id="skeleton" }` per qualifying death
