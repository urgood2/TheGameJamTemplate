...
vec4 applyOverlay(vec2 atlasUV) {
    ...
    // Base refraction strength: 0.02 provides subtle chromatic aberration effect
    ...
    vec2 localR = clamp(clampedLocal + localRefractDir * 1.0, 0.0, 1.0);
    vec2 localG = clamp(clampedLocal + localRefractDir * 0.5, 0.0, 1.0);
    vec2 localB = clamp(clampedLocal - localRefractDir * 0.5, 0.0, 1.0);
    ...
    // Edge highlight threshold: 0.02 creates sharp, bright edges at facet boundaries
    float edgeHighlight = smoothstep(0.02, 0.0, edgeDist);
    // Edge glow threshold: 0.08 creates softer secondary glow around edges
