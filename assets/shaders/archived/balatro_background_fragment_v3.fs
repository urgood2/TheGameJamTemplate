...
vec4 effect(vec2 screenSize, vec2 screen_coords)
{
    ...
    // ✨ Animate the blend weights using spatial-time noise
    float noise = sin(uv.x * 0.1 + uv.y * 0.13 + iTime * 0.2) * 
                  cos(uv.x * 0.07 - uv.y * 0.11 - iTime * 0.3);
    noise = 0.5 + 0.5 * noise; // remap to 0..1
    ...
    // 🎨 Mix color weights using animated noise
    c1p *= 1.0 - 0.3 * noise;
    c2p *= 1.0 + 0.3 * noise;
    c3p *= 1.0 + 0.15 * sin(iTime + uv.x * 0.3);
...
void main()
{
    ...
    vec2 parallax = vec2(
        sin(iTime * 0.1 + fragTexCoord.y * 5.0),
        cos(iTime * 0.07 + fragTexCoord.x * 4.0)
    ) * (0.005 + spin_amount * 0.01);

    vec2 uv_shifted = fragTexCoord + parallax;

    vec2 screen_coords = (polar_coordinates
        ? polar_coords(uv_shifted, polar_center, polar_zoom, polar_repeat)
        : uv_shifted) * screenSize;
