...
/**
 * @brief Constructs a PhysicsWorld object and initializes the Chipmunk2D
 * physics space.
 *
 * This constructor initializes a new Chipmunk2D physics space, sets the gravity
 * and iterations, and associates the PhysicsWorld with an `entt::registry` for
 * managing entities and components.
 *
 * @param registry Pointer to the `entt::registry` used to manage entities and
 * components in the game or simulation.
 * @param meter (Unused in this implementation) A scaling factor representing
 * the number of pixels per meter. Typically used for scaling physics
 * calculations.
 * @param gravityX The X component of the gravitational force applied in the
 * physics simulation.
 * @param gravityY The Y component of the gravitational force applied in the
 * physics simulation.
 *
 * @details
 * - The physics space is initialized using `cpSpaceNew`.
 * - Gravity is set using `cpSpaceSetGravity`, with the vector `(gravityX,
 * gravityY)` specifying the direction and strength.
 * - The default number of solver iterations is set to 10 using
 * `cpSpaceSetIterations`.
 * - The `registry` pointer allows the physics system to interact with entities
 * and their associated components.
 *
 * @note
 * The `meter` parameter is not currently used in this implementation but can be
 * incorporated if physics-world-to-screen scaling is required in the future.
 *
 * @example
 * // Example usage to create a PhysicsWorld object with gravity pointing
 * downward entt::registry registry; PhysicsWorld physicsWorld(&registry, 1.0f,
 * 0.0f, -9.8f);
 */
 ...
 *
 * @details
 * - Uses `cpSpaceStep` to advance the Chipmunk2D physics simulation by
 * `deltaTime`.
 * - Updates the positions, velocities, and interactions of all objects in the
 * physics space.
 * - This method should be called once per frame to ensure consistent simulation
 * results.
 *
 * @note
 * - A smaller `deltaTime` increases simulation precision but may require more
 * computational resources.
 * - Ensure `deltaTime` is consistent across frames for predictable physics
 * behavior.
 *
 * @example
 ...
 *
 * @details
 * - Resets the event tracking lists (`collisionEnter`, `collisionExit`,
 * `triggerEnter`, and `triggerExit`) to prepare for the next simulation step.
 * - This method should be called after processing all collision and trigger
 * events in the current frame.
 * - Helps maintain a clean state for event handling between updates.
 *
 * @note
 * - The persistent state of collisions and triggers should be handled elsewhere
 * if needed, as this method clears the current frame's data.
 ...
 * - The gravity vector affects all dynamic bodies unless they have their
 * gravity scale set to 0.
 * - This method can be used to simulate different gravitational environments or
 * adjust gravity during gameplay.
 *
 * @note
 * - Default gravity is typically set during the construction of the
 * `PhysicsWorld` object.
...
// Chipmunk2D doesn't use a "meter" system like Box2D or Love2D, but we can
// simulate it by scaling forces or coordinates. You could create a method to
// manage this scaling for consistency.
 ...
 *
 * @details
 * - Sets up a collision handler for processing collisions of objects with
 * collision type `1`.
 * - Registers callback functions to handle collision start (`beginFunc`) and
 * collision end (`separateFunc`).
 * - Each callback function delegates processing to the appropriate member
 * functions of the `PhysicsWorld` class: `OnCollisionBegin` and
 * `OnCollisionEnd`.
 *
 * @note
 * - This method assumes a collision type of `1`. If multiple collision types
 * are used, additional handlers may need to be added.
 * - The callbacks provide custom logic for detecting and responding to
 * collisions between objects.
 * - The `userData` field in the handler is used to associate the `PhysicsWorld`
 * instance with the callbacks.
 *
 * @example
...
void PhysicsWorld::SetCollisionCallbacks() {
  ...
  cpCollisionHandler *handler =
      cpSpaceAddCollisionHandler(space, 1, 1); // Collision type 1 with itself
...
/**
 * @brief Handles the beginning of a collision between two shapes in the physics
 * world.
 *
 * @param arb A pointer to the Chipmunk arbiter object that describes the
 * collision.
 *
 * @details
 * - This function is called whenever two shapes in the physics simulation start
 * colliding.
 * - It distinguishes between physical collisions and trigger events (sensor
 * interactions).
 * - For physical collisions:
 *   - Verifies collision category and mask compatibility using the filters.
 *   - Adds a `CollisionEvent` to the `collisionEnter` and `collisionActive`
 * lists for tracking.
 * - For trigger interactions:
 *   - Tracks trigger events in `triggerEnter` and `triggerActive`.
 *
 * @note
 * - Assumes the collision categories and masks are preconfigured for the
 * shapes.
 * - Trigger and physical collision logic are handled separately to allow for
 * different game mechanics.
 * - User data associated with the shapes is used to identify objects during
 * collision handling.
 *
 * @example
 ...
 *     handler->beginFunc = [](cpArbiter *arb, cpSpace *space, void *data) ->
 * cpBool { auto world = static_cast<PhysicsWorld *>(data);
 *         world->OnCollisionBegin(arb);
 *         return cpTrue;
 *     };
 * }
...
/**
 * @brief Handles the end of a collision between two shapes in the physics
 * world.
 *
 * @param arb A pointer to the Chipmunk arbiter object that describes the
 * collision.
 *
 * @details
 * - This function is called whenever two shapes in the physics simulation stop
 * colliding.
 * - It distinguishes between physical collisions and trigger events (sensor
 * interactions).
 * - For physical collisions:
 ...
 *
 * @details
 * - This function modifies the collision filter masks of the specified tags to
 * enable interactions between them.
 * - It updates the `collisionTags` map, which stores the collision categories
 * and masks for each tag.
 * - The tags specified in the `tags` vector will be added to the collision mask
 * of `tag1`.
 * - Once the collision mask is updated, objects with these tags will be able to
 * collide with each other during the physics simulation.
 *
 * @note
 * - The collision filter allows fine-grained control over which objects can
 * collide with each other.
 * - The `collisionTags` map must be populated with tags and their corresponding
 * categories prior to calling this function.
 *
 * @example
 ...
 * @param tags A vector of tags that will be removed from the collision mask of
 * `tag1`.
 *
 * @details
 * - This function modifies the collision filter masks of the specified tags to
 * disable interactions between them.
 * - It updates the `collisionTags` map, which stores the collision categories
 * and masks for each tag.
 * - The tags specified in the `tags` vector will be removed from the collision
 * mask of `tag1`, preventing collisions between these tags.
 * - Once the mask is updated, objects with these tags will no longer be able to
 * collide with objects having the `tag1` tag during the physics simulation.
 *
 * @note
 * - The collision filter allows fine-grained control over which objects can
 * collide with each other.
 ...
 *
 * @param tag1 The first tag that will have its triggers enabled with the tags
 * specified in the `tags` vector.
 * @param tags A vector of tags that will be added to the trigger list for
 * `tag1`.
 *
 * @details
 * - This function modifies the trigger configuration of the specified tags to
 * enable interactions between them.
 * - It updates the `triggerTags` map, which stores the trigger categories and
 * their respective trigger lists for each tag.
 * - The tags specified in the `tags` vector will be added to the trigger list
 * of `tag1`, allowing trigger interactions between the two sets of tags.
 * - Once this function is called, objects with `tag1` will be able to trigger
 * events with objects having the specified tags in the `tags` vector.
 *
 * @note
 * - The trigger system allows objects to detect when they enter or exit
 * specific regions or interact in a non-colliding manner.
 * - The `triggerTags` map must be populated with tags and their corresponding
 * categories and triggers before calling this function.
 *
 * @example
 ...
 *
 * @param tag1 The first tag whose trigger interactions with the tags in the
 * `tags` vector will be disabled.
 ...
 * - It updates the `triggerTags` map, which stores the trigger categories and
 * their respective trigger lists for each tag.
 * - The tags specified in the `tags` vector will be removed from the trigger
 * list of `tag1`, effectively disabling the trigger interactions between `tag1`
 * and the specified tags.
 * - Once this function is called, objects with `tag1` will no longer trigger
 * events with objects having the specified tags in the `tags` vector.
 *
 * @note
 * - The trigger system allows objects to detect when they enter or exit
 ...
 * @return A constant reference to a vector of `CollisionEvent` objects
 * representing the collision events between the two types.
 *
 * @details
 * - This function provides access to the list of collision events between two
 * specified types. It returns the events stored in the `collisionEnter` map,
 * which is indexed by the concatenated `type1:type2` key.
 * - Collision events are populated when a collision between objects of the
 * specified types occurs.
 * - The returned vector will contain details about each collision event, such
 * as the objects involved and any other relevant data.
 *
 * @note
 * - The `collisionEnter` map is populated when a collision begins, and the
 * events are stored for later access.
 * - If no collision between the specified types has occurred, the function will
...
/**
 * @brief Retrieves the list of objects that triggered an event between two
 * specified types.
 *
 ...
 * @return A constant reference to a vector of pointers to objects that
 * triggered the event between the specified types.
 *
 * @details
 * - This function provides access to the list of objects that triggered a
 * sensor event between two specified types. It returns the objects stored in
 * the `triggerEnter` map, which is indexed by the concatenated `type1:type2`
 * key.
 * - Trigger events are populated when one object enters the trigger area of
 * another object of the specified type.
 * - The returned vector will contain pointers to the objects involved in the
 * trigger event.
 *
 * @note
 * - The `triggerEnter` map is populated when a trigger event begins, and the
 * objects that triggered the event are stored for later access.
 * - If no trigger event between the specified types has occurred, the function
...
/**
 * @brief Sets the collision and trigger tags for the objects in the physics
 * world.
 *
 * @param tags A vector of tags that will be assigned to the collision and
 * trigger categories.
 *
 * @details
 * - This function initializes and clears the `collisionTags` and `triggerTags`
 * maps, which are responsible for storing the collision and trigger categories
 * for various tags.
 * - The `tags` vector contains the list of tags that will be assigned to each
 * category.
 * - Each tag is assigned a unique category, which is used to determine
 * collision interactions and trigger events.
 * - The categories are assigned incrementally as powers of 2, starting from 1.
 * - The `collisionTags` map stores information related to collision categories,
 * while the `triggerTags` map stores trigger categories.
 *
 * @note
 * - This function clears any previously set tags and categories, so calling it
 * will overwrite existing settings.
 * - The categories are assigned using bitwise shifting, ensuring that each tag
...
void PhysicsWorld::AddCollisionTag(const std::string &tag) {
  ...
  int category = 1;
  while (collisionTags.count(
      categoryToTag[category])) // Find the next available category
    category <<= 1;
...
/**
 * @brief Performs a raycast in the physics world and returns the hit
 * information.
 *
 ...
 *
 * @return A vector of `RaycastHit` objects that contain information about the
 * shapes hit by the ray.
 *
 * @details
 * - This function performs a raycast between the points `(x1, y1)` and `(x2,
 * y2)` using Chipmunk's `cpSpaceSegmentQuery` function.
 * - It queries the physics space for any shapes that the ray intersects and
 * returns a list of the hit shapes along with additional details such as the
 * collision point, the normal vector at the point of contact, and the fraction
 * of the ray's length at the hit location.
 * - The query checks all shapes in the space using the `CP_SHAPE_FILTER_ALL`
 * filter, meaning that it will consider all shape types.
 * - A callback function processes each hit shape and stores the results in the
 * provided vector of `RaycastHit` objects.
 *
 * @note
 * - Raycasting is commonly used for line-of-sight checks, bullet simulations,
 * or any scenario where you need to determine which objects a ray intersects.
 * - This method is highly efficient for checking multiple shapes in a single
 * query, making it ideal for scenarios like determining line-of-sight or
 * interactions in physics-based simulations.
 * - The fraction (`alpha`) value represents how far along the ray the collision
 * occurred. A fraction of `0.0` means the collision happened at the start of
 * the ray, and `1.0` means the collision occurred at the end of the ray.
 *
 * @example
...
/**
 * @brief Retrieves all objects within a specified rectangular area in the
 * physics world.
 *
 ...
 *
 * @return A vector of void* pointers, each representing the user data of the
 * shapes within the specified area.
 *
 * @details
 * - This method performs a bounding box query to find all shapes within a
 * rectangular region defined by the coordinates `(x1, y1)` and `(x2, y2)`.
 * - It uses Chipmunk's `cpSpaceBBQuery` to query the physics space for all
 * shapes that intersect the given bounding box.
 * - The query will return all shapes in the area regardless of shape type or
 * category, as the filter `CP_SHAPE_FILTER_ALL` is used.
 * - The `userData` of each shape (if set) is collected and returned in the
 * resulting vector. The `userData` can be any type of data associated with the
 * shape, often representing game objects or other components.
 *
 * @note
 * - The returned vector contains raw pointers (`void*`) to the `userData` of
 * the shapes. You need to cast these to the appropriate types based on the
 * specific game object or component stored in the `userData`.
 * - This method is useful for operations like detecting all objects within a
 * certain area, applying effects, or checking for collisions or interactions
 * with multiple objects.
 *
 * @example
 * // Example usage of getting all objects within an area:
 * std::vector<void *> objectsInArea = world.GetObjectsInArea(0.0f, 0.0f,
 * 100.0f, 100.0f); for (void *object : objectsInArea) {
...
/**
 * @brief Adds a rectangular shape to the physics world and applies a collision
 * filter.
 *
 ...
 *
 * @details
 * This method creates a rectangular shape with the specified dimensions
 * (`width` and `height`) and attaches it to the provided `cpBody`.
 * - It applies the collision filter based on the `tag` provided, which
 * determines how this shape interacts with other shapes in the physics world.
 * - It assigns the `userData` of the shape (optional), which can be used to
 * store references to game objects or other related data.
 * - The shape is added to the physics world (`space`) for simulation.
 *
 * @note
 * - The `tag` provided will be used to apply the collision filter, which is
 * critical in determining which shapes will interact with one another based on
 * their categories and masks.
 * - `userData` is set to the `PhysicsWorld` object (`this`) for convenience,
 * but it can be modified to store any type of data, depending on the game's
 * design (such as a pointer to a specific game object).
 *
 * @example
 ...
 * // This adds a rectangular shape of size 10x5 to the `body` with the tag
 * "player"
 */
 ...
 *
 * This function creates a physics body and shape based on the specified
 * parameters, applies collision filtering, and registers the collider as a
 * component for the specified entity using the EnTT registry.
 *
 ...
 *            The tag should correspond to a category defined in the collision
 * filtering setup.
 ...
 * @param a The primary dimension (e.g., width for rectangles, radius for
 * circles). Ignored if `points` is provided.
 * @param b The secondary dimension (e.g., height for rectangles). Ignored if
 * `points` is provided.
 * @param c Used for chain shapes as the x-coordinate of the third vertex.
 * Ignored for other shape types or if `points` is provided.
 * @param d Used for chain shapes as the y-coordinate of the third vertex.
 * Ignored for other shape types or if `points` is provided.
 * @param isSensor A boolean flag indicating whether the collider is a sensor.
 *                 Sensors detect overlaps but do not produce physical
 * responses.
 * @param points (Optional) A vector of points defining the vertices of the
 * shape. Used for "polygon" or "chain" shape types. Overrides the `a`, `b`,
 * `c`, and `d` parameters for these types.
 *
 * @throws std::invalid_argument If an unsupported `shapeType` is provided.
 *
 * This function performs the following steps:
 * 1. Creates a physics body using `MakeSharedBody`, with a mass of 0 if it's a
 * sensor (static body).
 ...
 * 3. Applies collision filtering to the shape using the specified `tag` via
 ...
 * - **Circle:** Renders as a filled circle using the radius and offset from the
 * shape.
 * - **Rectangle:** Renders as a filled rectangle using the bounding box
 * dimensions.
 * - **Polygon:** Renders as a series of connected lines forming the polygon
 * shape.
 ...
 * - **Velocity:** Renders as a yellow line originating from the collider's
 * position.
 * - **Angular Velocity:** Renders as a rotating orange line around the
 * collider's position.
 *
 * @details
 * - The function uses `collider.debugDraw` to determine whether to render a
 * specific collider.
 * - It retrieves the `ColliderShapeType` from the `ColliderComponent` to decide
 * the rendering logic.
 * - Positions and velocities are transformed from physics space to rendering
 * space.
 * - All rendering operations use Raylib functions for visualization.
 *
 * @note
 * Ensure that `ColliderComponent` includes a `shapeType` variable of type
 * `ColliderShapeType` to classify the shape of the collider. Also, ensure that
 * Raylib's rendering functions are properly integrated into the project.
 *
 * @example
 * // Enable debug drawing for a collider
 * registry.emplace_or_replace<ColliderComponent>(entity, colliderBody,
 * colliderShape, true);
 *
 ...
 *
 * This method calculates a desired velocity towards the target position and
 * adjusts the current velocity of the entity by applying a steering force. The
 * resulting movement is smooth and mimics seeking behavior, commonly used in AI
 * and game physics.
 *
 ...
 *
 * @details
 * - The function computes the desired velocity vector by normalizing the
 * direction towards the target point and scaling it by `maxSpeed`.
 * - The steering force is calculated as the difference between the desired
 * velocity and the entity's current velocity.
 * - This steering force is applied to the entity's physics body at its current
 * position.
 * - The behavior creates smooth and realistic seeking motion, often used in
 * steering behaviors for AI entities.
 *
 * **Key Steps:**
 * 1. Compute the desired velocity towards the target.
 * 2. Calculate the steering force by subtracting the current velocity from the
 * desired velocity.
 ...
 *
 * **Use Cases:**
 * - Guiding AI-controlled entities toward a specific location in the game
 * world.
 * - Implementing behaviors such as flocking, pathfinding, or basic autonomous
 * movement.
 *
 * **Example Usage:**
 * ```cpp
 * // Seek behavior for an entity to move towards the target position (300, 400)
 * with max speed 200 physicsWorld.Seek(playerEntity, 300.0f, 400.0f, 200.0f);
 * ```
 *
 * **Related Methods:**
 ...
 *
 * @note
 * This method assumes that the physics body is dynamic and can respond to
 * applied forces. The `maxSpeed` parameter determines the upper limit of the
 * velocity magnitude.
 */
...
/**
 * @brief Guides an entity toward a target point while slowing down as it
 * approaches the target.
 *
 * This method implements the "arrive" steering behavior, where an entity
 * accelerates towards a target point and gradually slows down within a
 * specified radius to avoid overshooting or abrupt stops.
 *
 * @param entity The entity whose movement is to be adjusted.
 * @param targetX The X-coordinate of the target position.
 * @param targetY The Y-coordinate of the target position.
 * @param maxSpeed The maximum speed the entity can achieve while moving towards
 * the target.
 * @param slowingRadius The radius around the target where the entity will begin
 * to slow down.
 *
 * @details
 * - The method calculates the distance to the target and adjusts the speed
 * proportionally if the entity is within the `slowingRadius`.
 * - Outside the `slowingRadius`, the entity moves at `maxSpeed`.
 * - The desired velocity is calculated based on the target position and
 * adjusted speed.
 * - A steering force is then applied to align the entity's velocity with the
 * desired velocity.
 *
 * **Key Steps:**
 * 1. Calculate the vector from the entity's current position to the target
 * (`toTarget`).
 ...
 * 3. Compute the desired velocity using the adjusted speed and direction to the
 * target.
 * 4. Calculate the steering force as the difference between the desired
 * velocity and the current velocity.
 * 5. Apply the steering force to the entity's physics body at its current
 * position.
 *
 * **Requirements:**
 * - The entity must have a `ColliderComponent` with a valid `cpBody` reference.
 ...
 * - Implementing natural stopping behaviors for vehicles, characters, or
 * projectiles.
 *
 * **Example Usage:**
 * ```cpp
 * // Move the entity towards the point (500, 500) with a max speed of 200 and a
 * slowing radius of 100. physicsWorld.Arrive(playerEntity, 500.0f, 500.0f,
 * 200.0f, 100.0f);
 * ```
 *
 * **Related Methods:**
 ...
 *
 * @note
 * This method ensures smooth deceleration near the target but does not directly
 * stop the entity unless the desired velocity becomes zero. A separate method
 * may be required for precise stopping.
 */
...
/**
 * @brief Causes an entity to move in a wandering pattern by introducing
 * randomness to its movement direction.
 *
 * This method implements the "wander" steering behavior, where an entity moves
 * in an unpredictable yet smooth trajectory by randomly adjusting its steering
 * direction within a defined range.
 *
 * @param entity The entity to apply the wandering behavior.
 * @param wanderRadius The radius of the imaginary circle around which the
 * entity wanders.
 * @param wanderDistance The forward distance from the entity's current position
 * to the center of the wandering circle.
 ...
 *
 * @details
 * - The wander behavior is achieved by creating an imaginary circle in front of
 * the entity.
 * - A displacement point on the circle is adjusted by a random jitter each
 * frame, causing the entity to steer slightly differently each time.
 * - The target point for the entity is the sum of the circle's center and the
 * adjusted displacement point.
 * - The method uses the existing `Seek` function to guide the entity toward the
 ...
 *    - Use trigonometric functions to compute the displacement based on the
 * updated angle.
 * 4. Compute the target point by summing the circle center and the
 * displacement.
 * 5. Call the `Seek` method to steer the entity toward the target point with
 ...
 * - Simulating animals, vehicles, or characters that roam the game world
 * without specific destinations.
 *
 * **Example Usage:**
 * ```cpp
 * // Apply wandering behavior with a radius of 50, distance of 100, jitter of
 * 0.2, and max speed of 150. physicsWorld.Wander(enemyEntity, 50.0f, 100.0f,
 * 0.2f, 150.0f);
 * ```
 *
 * **Related Methods:**
 * - `Seek`: Guides the entity toward a specific target point.
 * - `Arrive`: Moves the entity to a point with smooth deceleration near the
 * target.
 * - `Separate`: Prevents clustering by steering the entity away from nearby
 * objects.
 *
 * @note
 * The `wanderAngle` is static to ensure continuity in the wandering direction
 * between frames. This method should be called frequently (e.g., every frame)
 * for the effect to appear smooth.
 */
void PhysicsWorld::Wander(entt::entity entity, float wanderRadius,
  ...
  wanderAngle +=
      jitter * ((rand() % 200 - 100) / 100.0f); // Random jitter in radians
...
/**
 * @brief Applies a separation force to an entity to prevent it from clustering
 * too closely with other entities.
 *
 * This method implements the "separation" steering behavior, which ensures that
 * an entity maintains a minimum distance from other entities by applying a
 * repulsive force based on proximity.
 *
 ...
 * @param separationRadius The radius within which the separation force is
 * applied.
 * @param maxForce The maximum force applied to the entity to maintain
 * separation.
 *
 * @details
 * - The separation behavior helps entities avoid overlapping or clustering too
 * closely with others.
 * - The repulsive force is proportional to the inverse of the distance between
 * the entity and others, ensuring stronger repulsion for closer entities.
 * - The resulting force is a sum of all individual separation forces and is
 * applied as a single steering force to the entity.
 *
 * **Key Steps:**
 ...
 *    - If the other entity is within the `separationRadius`, compute a
 * normalized repulsive force.
 *    - Scale the force by the maximum force divided by the distance and add it
 * to the steering vector.
 * 4. Apply the accumulated steering force to the entity's body.
 *
 * **Requirements:**
 * - The `entity` and all entities in `others` must have a `ColliderComponent`
 * with a valid `cpBody` reference.
 * - The method should be called frequently (e.g., every frame) to maintain
 * continuous separation.
 *
 * **Use Cases:**
 * - Preventing AI entities from clustering or overlapping in crowded
 * environments.
 * - Simulating flocking behaviors, where separation is a key component
 * alongside alignment and cohesion.
 * - Ensuring characters, vehicles, or objects maintain a comfortable distance
 * in dynamic scenes.
 *
 * **Example Usage:**
 * ```cpp
 ...
 * - `Cohesion`: Moves the entity toward the average position of nearby
 * entities.
 * - `Wander`: Introduces random, lifelike movement to entities.
 *
 * @note
 * The force magnitude is clamped by `maxForce / distance` to ensure that closer
 * entities exert a stronger repulsion. Entities farther than `separationRadius`
 ...
 *
 * This method directly applies a force vector to an entity's physics body,
 * affecting its velocity and enabling dynamic movement. The force is applied at
 * the body's center of mass by default.
 *
 ...
 *
 * @details
 * - Forces are cumulative and directly modify the body's acceleration over
 * time, resulting in smooth, continuous motion.
 * - The entity must have a `ColliderComponent` that includes a valid `cpBody`
 * reference for the method to function properly.
 *
 * **Key Steps:**
 * 1. Retrieve the entity's `ColliderComponent` from the registry.
 * 2. Use `cpBodyApplyForceAtWorldPoint` to apply the specified force to the
 ...
 * **Example Usage:**
 * ```cpp
 * // Apply a force of 100 units in the X direction and 50 units in the Y
 * direction to an entity. physicsWorld.ApplyForce(playerEntity, 100.0f, 50.0f);
 * ```
 *
 * **Related Methods:**
 * - `ApplyImpulse`: Instantly applies an impulse to an entity, altering its
 ...
 *
 * @note
 * Unlike an impulse, a force is applied continuously over time. For one-time
 * instantaneous effects, consider using `ApplyImpulse` instead.
 */
 ...
 *
 * This method applies a one-time impulse to an entity's physics body, causing
 * an immediate change in its velocity. Unlike forces, which act over time,
 * impulses provide an instantaneous effect on the entity's motion.
 *
 ...
 * - The magnitude and direction of the impulse determine how the entity's
 * velocity is modified.
 * - The entity must have a `ColliderComponent` with a valid `cpBody` for the
 * method to execute properly.
 *
 * **Key Steps:**
 * 1. Retrieve the entity's `ColliderComponent` from the registry.
 * 2. Use `cpBodyApplyImpulseAtWorldPoint` to apply the specified impulse to the
 * body at its current position.
 *
 * **Use Cases:**
 ...
 * **Example Usage:**
 * ```cpp
 * // Apply an impulse of 200 units in the X direction and 100 units in the Y
 * direction to an entity. physicsWorld.ApplyImpulse(playerEntity, 200.0f,
 * 100.0f);
 * ```
 *
 * **Related Methods:**
 * - `ApplyForce`: Applies a continuous force to an entity over time, creating
 * smooth acceleration.
 * - `SetVelocity`: Directly sets the velocity of an entity's body.
 * - `ApplyTorque`: Applies a rotational impulse to the entity.
 *
 * @note
 * Impulses are ideal for scenarios requiring immediate changes in velocity,
 * such as simulating explosions or collisions.
 */
...
/**
 * @brief Sets damping on the velocity of a physics body associated with the
 * given entity.
 *
 * This function simulates damping by manually reducing the velocity of the
 * physics body. Damping slows down the motion of the body over time, simulating
 * the effect of air resistance or friction. This method modifies the velocity
 * of the body directly.
 *
 * @param entity The EnTT entity representing the object whose velocity should
 * be damped.
 * @param damping The damping factor to apply. A value between 0.0 (no damping)
 * and 1.0 (full stop).
 *
 * @note Damping is applied as a factor to the velocity. The resulting velocity
 * is calculated as: dampedVelocity = velocity * (1.0 - damping) For high
 * damping values (close to 1.0), the velocity will quickly reduce to near zero.
 *
 * @warning This function assumes the damping value is clamped between 0.0
 * and 1.0. Providing values outside this range may lead to undefined behavior.
 *
 * @example
 * PhysicsWorld physicsWorld;
 * physicsWorld.SetDamping(entity, 0.1f); // Apply 10% damping to the entity's
 * velocity.
 */
 ...
 *
 * Global damping affects all bodies in the physics simulation uniformly,
 * reducing their velocities over time. This simulates the effect of a medium,
 * like air or water, that provides resistance to movement. The damping is
 * applied during each physics step.
 *
 * @param damping The damping factor to apply globally. A value between 0.0 (no
 * damping) and 1.0 (full stop).
 *
 * @note The global damping is applied uniformly to all bodies in the physics
 * space. Individual body-specific damping can be applied separately using
 * `SetDamping`.
 *
 * @warning Ensure the damping value is within the range of 0.0 to 1.0. Values
 * outside this range may lead to undefined behavior.
 *
 * @example
 * PhysicsWorld physicsWorld;
 * physicsWorld.SetGlobalDamping(0.2f); // Apply 20% global damping to all
 * physics bodies.
 */
 ...
 *
 * This method directly sets the velocity of the body associated with the given
 * entity. The velocity is applied as a vector, with separate components for the
 * x-axis and y-axis.
 *
 ...
 *
 * @note This method overrides any existing velocity for the body. Use with
 * caution if other forces or accelerations are being applied to the body, as
...
/**
 * @brief Aligns an entity's velocity with the average velocity of its neighbors
 * within a given radius.
 *
 * This method calculates the average velocity of all neighboring entities
 * within the specified alignment radius and adjusts the velocity of the target
 * entity to align with this average. A maximum force is applied to limit the
 * steering adjustment, and the adjusted velocity is capped at the specified
 * maximum speed.
 *
 ...
 * @param maxForce The maximum steering force that can be applied to align the
 * velocity.
 *
 * @note This method assumes that all entities in `others` have valid
 * `ColliderComponent` instances and are part of the same physics world. Ensure
 * `alignRadius` is large enough to include relevant neighbors, but not so large
 * as to include distant entities that should not influence alignment.
 *
 * @example
 * // Align the velocity of `myEntity` with nearby entities within a radius of
 * 100 units. physicsWorld.Align(myEntity, nearbyEntities, 100.0f,
 * 200.0f, 50.0f);
 */
...
/**
 * @brief Adjusts an entity's position to move closer to the average position of
 * its neighbors within a given radius.
 *
 * This method calculates the average position of all neighboring entities
 * within the specified cohesion radius and applies a steering force to move the
 * target entity toward this average position. The steering behavior ensures
 * that the entity maintains cohesive movement with its neighbors, simulating
 * flocking or group dynamics.
 *
 ...
 * @param maxSpeed The maximum speed for the entity when moving toward the
 * average position.
 * @param maxForce The maximum steering force applied to adjust the entity's
 * position.
 *
 * @note The cohesion behavior uses the `Seek` method to steer the entity toward
 * the calculated average position. Ensure that `cohesionRadius` is large enough
 * to include relevant neighbors but not too large to incorporate distant
 * entities that should not influence cohesion.
 *
 * @example
...
void PhysicsWorld::Cohesion(entt::entity entity,
  ...
  if (neighborCount > 0) {
    ...
    Seek(entity, averagePosition.x, averagePosition.y,
         maxSpeed); // Seek method already exists in your implementation
  }
}

/**
 * @brief Ensures an entity stays above a specified vertical boundary by
 * applying a corrective force.
 *
 * This method checks if the entity's position is below the given vertical
 * boundary (`minY`). If it is, a proportional upward force is applied to push
 * the entity back above the boundary. This can be used to implement boundaries
 * for gameplay areas or keep objects within a defined region.
 *
 * @param entity The EnTT entity to enforce the boundary on.
 * @param minY The minimum vertical position (y-coordinate) the entity is
 * allowed to move below.
 *
 * @note The force applied is proportional to the distance the entity has moved
 * below the boundary, creating a smooth corrective behavior.
 *
 * @example
...
void PhysicsWorld::EnforceBoundary(entt::entity entity, float minY) {
  ...
  if (position.y > minY) {
    cpVect force = cpv(
        0, -1000 * (position.y -
                    minY)); // Apply a force proportional to the displacement
    cpBodyApplyForceAtWorldPoint(collider.body.get(), force, position);
  }
}

/**
 * @brief Accelerates an entity toward the current position of the mouse
 * pointer.
 *
 * This method calculates the direction from the entity to the mouse position
 * and applies an acceleration force toward that direction. The resulting
 * velocity is clamped to a specified maximum speed.
 *
 ...
 * @param maxSpeed The maximum speed the entity can achieve while moving toward
 * the mouse position.
 *
 * @note This method requires the use of the `AccelerateTowardAngle` function,
 * which handles acceleration toward a specific angle.
 *
 * @example
 * // Accelerate `myEntity` toward the mouse pointer with an acceleration of 500
 * units and a max speed of 250 units.
...
void PhysicsWorld::AccelerateTowardMouse(entt::entity entity,
  ...
  AccelerateTowardAngle(entity, angle, acceleration,
                        maxSpeed); // Assuming AccelerateTowardAngle exists
}

/**
 * @brief Accelerates an entity toward the position of another target entity.
 *
 * This method calculates the direction from the given entity to the target
 * entity and applies an acceleration force in that direction. The resulting
 * velocity is clamped to a specified maximum speed.
 *
 * @param entity The EnTT entity to apply the acceleration to.
 * @param target The target EnTT entity toward which the acceleration is
 * applied.
 * @param acceleration The magnitude of the acceleration force to be applied.
 * @param maxSpeed The maximum speed the entity can achieve while moving toward
 * the target.
 ...
 *
 * @example
 * // Accelerate `myEntity` toward `targetEntity` with an acceleration of 500
 * units and a max speed of 300 units.
 ...
 *
 * This method calculates the velocity vector based on the provided angle and
 * speed, then applies it directly to the entity's body. The entity will move
 * along a straight line in the specified direction at the given speed.
 *
 * @param entity The EnTT entity to move.
 * @param angle The direction in which the entity should move, specified in
 * radians.
 * @param speed The magnitude of the speed at which the entity should move.
 *
 * @note This method overrides the entity's current velocity with the newly
 * calculated velocity. Use this method for scenarios like projectiles or
 * constant motion in a given direction.
 *
 * @example
...
/**
 * @brief Rotates an entity toward the position of a target entity using a
 * linear interpolation value.
 *
 * This method computes the angle between the entity's current position and the
 * target's position, then adjusts the entity's current angle toward the target
 * angle using the specified lerp value.
 *
 ...
 *                  A value closer to 1 rotates the entity faster, while lower
 * values create a slower, smoother rotation.
 *
 * @note This method is useful for implementing behaviors like homing
 * projectiles, AI entities aiming at a target, or orienting objects in a
 * physics simulation toward a goal.
 *
 * @example
...
/**
 * @brief Rotates an entity toward a specific point using a linear interpolation
 * value.
 *
 * This method calculates the angle between the entity's current position and a
 * target point, then adjusts the entity's current angle toward the target angle
 * using the specified lerp value.
 *
 * @param entity The EnTT entity to rotate.
 ...
 * values create a slower, smoother rotation.
 *
 * @note This method is useful for aligning entities toward a fixed point, such
 * as in aiming mechanics or orienting objects in a physics simulation toward a
 * static target.
 *
 * @example
 * // Rotate `entity` toward point (100.0f, 200.0f) with a smooth interpolation
 * of 0.05. physicsWorld.RotateTowardPoint(entity, 100.0f, 200.0f, 0.05f);
 */
...
/**
 * @brief Rotates an entity toward the current mouse position using a linear
 * interpolation value.
 *
 * This method calculates the angle between the entity's current position and
 * the mouse position, then adjusts the entity's current angle toward the target
 * angle using the specified lerp value.
 *
 * @param entity The EnTT entity to rotate.
 * @param lerpValue The interpolation value controlling the speed of rotation.
 ...
 * values create a slower, smoother rotation.
 *
 * @note This method assumes that the mouse position is retrieved using Raylib's
 * `GetMousePosition` function and is appropriate for 2D scenarios where the
 * entity needs to follow or aim toward the mouse.
 *
 * @example
 * // Rotate `entity` smoothly toward the mouse position with a lerp value of
 * 0.1. physicsWorld.RotateTowardMouse(entity, 0.1f);
 */
...
/**
 * @brief Rotates an entity toward the direction of its current velocity using
 * linear interpolation.
 *
 * This method calculates the angle of the entity's velocity vector and adjusts
 * the entity's current angle toward this target angle using the specified lerp
 * value.
 *
 * @param entity The EnTT entity to rotate.
 * @param lerpValue The interpolation value controlling the speed of rotation.
 *                  Higher values result in faster rotation, while lower values
 * create smoother, slower rotation.
 *
 * @note This method assumes that the entity has a valid velocity. If the
 * velocity vector length is zero, the method does nothing.
 *
 * @example
 * // Rotate the entity smoothly toward its velocity direction with a lerp value
 * of 0.1. physicsWorld.RotateTowardVelocity(entity, 0.1f);
 */
...
/**
 * @brief Accelerates an entity in a specified direction while clamping its
 * velocity to a maximum speed.
 *
 * This method applies a force to the entity's body in the direction specified
 * by the angle. It also ensures that the resulting velocity does not exceed the
 * maximum speed by clamping it.
 *
 * @param entity The EnTT entity to accelerate.
 * @param angle The direction of acceleration in radians. The angle is measured
 * counterclockwise from the positive x-axis.
 * @param acceleration The magnitude of the force to be applied in the specified
 * direction.
 * @param maxSpeed The maximum speed the entity is allowed to reach. If the
 * resulting velocity exceeds this limit, it is clamped to the maximum speed.
 *
 * @note The method assumes the entity has a valid `ColliderComponent` and that
 * its body has already been added to the physics space.
 *
 * @example
 * // Accelerate the entity at a 45-degree angle with a force of 100 units and a
 * max speed of 200 units. physicsWorld.AccelerateTowardAngle(entity, M_PI / 4,
 * 100.0f, 200.0f);
 */
...
/**
 * @brief Accelerates an entity toward a specified target point, limiting its
 * velocity to a maximum speed.
 *
 * This method calculates the direction from the entity's current position to
 * the target point and applies a force in that direction. The resulting
 * velocity is clamped to the specified maximum speed.
 *
 * @param entity The EnTT entity to accelerate.
 * @param targetX The x-coordinate of the target point.
 * @param targetY The y-coordinate of the target point.
 ...
 * @note The method assumes the entity has a valid `ColliderComponent` and that
 * its body has already been added to the physics space.
 * @note This method relies on the `AccelerateTowardAngle` method to handle the
 * actual acceleration and velocity clamping.
 *
 * @example
 * // Accelerate the entity toward the point (100, 200) with a force of 150
 * units and a max speed of 300 units.
...
/**
 * @brief Moves an entity toward a specified target point at a given speed or
 * within a specified maximum time.
 *
 * This method calculates the direction from the entity's current position to
 * the target point and sets its velocity to move toward the point. If `maxTime`
 * is specified, the speed is adjusted to ensure the entity reaches the target
 * within the given time.
 *
 * @param entity The EnTT entity to move.
 * @param targetX The x-coordinate of the target point.
 * @param targetY The y-coordinate of the target point.
 * @param speed The desired speed of the entity. If `maxTime` is provided, this
 * value is recalculated based on the distance.
 * @param maxTime The maximum time (in seconds) for the entity to reach the
 * target. If set to 0, the provided speed is used.
 *
 * @note This method directly sets the velocity of the entity's physics body and
 * does not account for acceleration or deceleration.
 *
 * @example
 * // Move the entity toward the point (300, 400) at a speed of 100 units per
 * second. physicsWorld.MoveTowardPoint(entity, 300.0f, 400.0f, 100.0f, 0.0f);
 *
 * // Move the entity toward the point (300, 400) and ensure it reaches the
 * target in 2 seconds. physicsWorld.MoveTowardPoint(entity, 300.0f, 400.0f,
 * 0.0f, 2.0f);
 */
...
/**
 * @brief Moves an entity toward the current mouse position at a specified speed
 * or within a given maximum time.
 *
 * This method retrieves the current mouse position (using Raylib's
 * `GetMousePosition`) and calculates the direction from the entity's position
 * to the mouse. It then calls `MoveTowardPoint` to set the velocity for moving
 ...
 * value is recalculated based on the distance.
 * @param maxTime The maximum time (in seconds) for the entity to reach the
 * mouse position. If set to 0, the provided speed is used.
 *
 * @note This method directly modifies the velocity of the entity's physics body
 * and does not account for acceleration or deceleration.
 *
 * @example
 * // Move the entity toward the mouse position at a speed of 100 units per
 * second. physicsWorld.MoveTowardMouse(entity, 100.0f, 0.0f);
 *
 * // Move the entity toward the mouse position and ensure it reaches the target
 * in 1.5 seconds. physicsWorld.MoveTowardMouse(entity, 0.0f, 1.5f);
 */
...
/**
 * @brief Locks the vertical movement of an entity, allowing it to move only
 * horizontally.
 *
 * This method retrieves the current velocity of the entity and sets the
 * vertical velocity component to zero, effectively locking the entity's
 * movement along the vertical axis.
 *
 * @param entity The EnTT entity whose vertical movement is to be locked.
 *
 * @note This method modifies the velocity of the physics body directly. It does
 * not affect acceleration or forces applied to the body, but any applied
 * vertical force will result in no vertical movement.
 *
 * @example
...
void PhysicsWorld::LockHorizontally(entt::entity entity) {
  ...
  cpBodySetVelocity(collider.body.get(),
                    cpv(velocity.x, 0)); // Maintain horizontal velocity only
}

/**
 * @brief Locks the horizontal movement of an entity, allowing it to move only
 * vertically.
 *
 * This method retrieves the current velocity of the entity and sets the
 * horizontal velocity component to zero, effectively locking the entity's
 ...
 * @note This method modifies the velocity of the physics body directly. It does
 * not affect acceleration or forces applied to the body, but any applied
 * horizontal force will result in no horizontal movement.
 *
 * @example
...
void PhysicsWorld::LockVertically(entt::entity entity) {
  ...
  cpVect velocity = cpBodyGetVelocity(collider.body.get());
  cpBodySetVelocity(collider.body.get(),
                    cpv(0, velocity.y)); // Maintain vertical velocity only
}

/**
 * @brief Sets the angular velocity of an entity's physics body.
 *
 * This method directly updates the angular velocity of the specified entity,
 * allowing it to rotate at a specified rate. Angular velocity is measured in
 * radians per second.
 *
 ...
 *
 * @note This method overrides any current angular velocity and does not take
 * into account any torque or angular damping applied to the entity's physics
 * body.
 *
 * @example
 * // Set the entity to rotate at π/2 radians per second (90 degrees per
 * second). physicsWorld.SetAngularVelocity(entity, M_PI / 2.0f);
 *
 ...
 *
 * This function reduces the angular velocity of the specified entity's physics
 * body by a specified damping factor. Since Chipmunk 2D does not directly
 * support angular damping, this implementation applies the effect manually by
 * scaling the angular velocity on each call.
 *
 * @param entity The entity whose angular damping will be applied.
 * @param angularDamping A factor between 0.0f and 1.0f that determines the
 * damping effect. A value of 0.0f results in no damping, while 1.0f applies
 * full damping (stops rotation instantly).
 *
 * @note This method is typically called every frame or physics update to create
 * a continuous damping effect. The damping factor should be small (e.g., 0.01f)
 * to avoid abrupt changes in angular velocity.
 *
 * @example
...
/**
 * @brief Retrieves the current angle (rotation) of the specified entity in
 * radians.
 *
 * This function accesses the physics body associated with the specified entity
 * and returns its current rotation angle in radians. The angle represents the
 * orientation of the body in the 2D space relative to the x-axis.
 *
 * @param entity The entity whose rotation angle is to be retrieved.
 * @return float The angle of the entity in radians.
 *
 * @note The angle is measured in radians. To convert it to degrees, use a
 * conversion factor: degrees = radians * (180.0 / M_PI).
 *
 * @example
 ...
 *
 * This function updates the angle (orientation) of the physics body associated
 * with the specified entity. The angle determines the body's rotation in 2D
 * space relative to the x-axis.
 *
 * @param entity The entity whose rotation angle is to be set.
 * @param angle The new angle in radians.
 *
 * @note
 * - The angle is measured in radians. To convert from degrees to radians, use a
 * conversion factor: radians = degrees * (M_PI / 180.0).
 * - Directly setting the angle may conflict with ongoing physics interactions.
 * Use this carefully to avoid unexpected behavior during simulation.
 *
 * @example
 ...
 *
 * Restitution determines how much kinetic energy is conserved during a
 * collision. A higher value means the object will bounce more after a
 * collision, while a lower value reduces the bounce effect.
 *
 * @param entity The entity whose collider's restitution is to be set.
 * @param restitution The restitution value. This should typically be between
 * 0.0 (no bounce) and 1.0 (perfect bounce).
 *
 * @note
 * - Values greater than 1.0 can cause exaggerated bouncing effects but may lead
 * to unstable physics behavior.
 * - Setting the restitution does not affect ongoing collisions; it only applies
 * to future collisions.
 *
 * @example
 ...
 *
 * Friction determines how much resistance the collider has when sliding against
 * another surface. A higher value increases resistance to motion, while a lower
 * value reduces resistance.
 *
 * @param entity The entity whose collider's friction is to be set.
 * @param friction The friction coefficient. Typical values range from 0.0 (no
 * friction) to 1.0 (high friction). Values greater than 1.0 are possible for
 * extra resistance.
 *
 * @note
 * - Friction is a multiplicative property between two colliders during contact.
 * The combined friction is determined by both colliders' friction coefficients.
 * - Setting friction affects future physics interactions and does not
 * retroactively modify ongoing collisions.
 *
 * @example
 ...
 *
 * This method directly modifies the angular velocity of the collider by
 * calculating the effect of the given angular impulse using the collider's
 * moment of inertia.
 *
 * @param entity The entity whose collider receives the angular impulse.
 * @param angularImpulse The angular impulse to be applied, measured in units of
 * angular momentum (e.g., N·m·s).
 *
 * @note
 * - The angular impulse results in an instantaneous change in the angular
 * velocity of the body.
 * - The body's moment of inertia must be non-zero for this calculation to work
 * as expected.
 *
 * @example
 ...
 *
 * This method allows you to manually control the activation state of a body in
 * the physics simulation. If the body is set to awake, it will participate in
 * the simulation. If set to sleep, it will be excluded from the simulation
 * until explicitly reactivated or affected by external forces.
 *
 ...
 *              - `false`: Put the body to sleep, excluding it from simulation
 * until reactivated.
 *
 * @note
 * - Awake bodies are actively simulated and may interact with other objects in
 * the world.
 * - Sleeping bodies are idle and do not consume processing resources.
 * - A sleeping body will automatically wake up if touched by another active
 * body or explicitly activated.
 *
 * @example
...
/**
 * @brief Retrieves the current position of the specified entity's collider in
 * the physics world.
 *
 * This method returns the position of the body associated with the specified
 * entity as a `cpVect`, representing the coordinates in the physics world. The
 * position is typically in the simulation's coordinate system.
 *
 ...
 *
 * @note
 * - The position is relative to the physics world and may differ from other
 * game-world coordinate systems if a transformation or scaling factor is
 * applied.
...
/**
 * @brief Sets the position of the specified entity's collider in the physics
 * world.
 *
 * This method updates the position of the body associated with the given entity
 * to the specified coordinates. It directly modifies the collider's position,
 * effectively "teleporting" the entity in the physics simulation.
 *
 ...
 *
 * @note
 * - This operation does not affect the body's velocity or acceleration, and may
 * cause unexpected behavior if the entity is part of an active simulation
 * (e.g., collisions or forces).
 * - Avoid using this function frequently during simulation as it bypasses the
 * physics engine's continuous motion calculations.
 *
 * @example
...
/**
 * @brief Moves the specified entity's collider horizontally toward the mouse
 * position.
 *
 * This method adjusts the collider's horizontal velocity to move it toward the
 * x-coordinate of the mouse position. The vertical velocity remains unaffected.
 *
 * @param entity The entity whose collider is being moved.
 * @param speed The desired horizontal speed. If `maxTime` is provided, this
 * value will be overridden.
 * @param maxTime The time (in seconds) within which the entity should reach the
 * mouse's x-coordinate. If set to a value greater than 0, it calculates the
 * speed required to reach the target within the specified time.
 *
 * @note
 * - If the mouse position's x-coordinate is already close to the entity's
 * position, the calculated speed will be small, resulting in slower movement.
 * - The vertical velocity of the entity is preserved during this operation.
 *
 * @example
 * // Move an entity horizontally toward the mouse at a constant speed of 5.0f
 * units per second MoveTowardsMouseHorizontally(entity, 5.0f, 0.0f);
 *
 * @example
 * // Move an entity horizontally toward the mouse within 2 seconds, regardless
 * of the distance MoveTowardsMouseHorizontally(entity, 0.0f, 2.0f);
 */
void PhysicsWorld::MoveTowardsMouseHorizontally(entt::entity entity,
  ...
  if (maxTime > 0.0f) {
    float distance =
        fabs(mousePosition.x - position.x); // Only horizontal distance
...
/**
 * @brief Moves the specified entity's collider vertically toward the mouse
 * position.
 *
 * This method adjusts the collider's vertical velocity to move it toward the
 * y-coordinate of the mouse position. The horizontal velocity remains
 * unaffected.
 *
 * @param entity The entity whose collider is being moved.
 * @param speed The desired vertical speed. If `maxTime` is provided, this value
 * will be overridden.
 * @param maxTime The time (in seconds) within which the entity should reach the
 * mouse's y-coordinate. If set to a value greater than 0, it calculates the
 * speed required to reach the target within the specified time.
 *
 * @note
 * - If the mouse position's y-coordinate is already close to the entity's
 * position, the calculated speed will be small, resulting in slower movement.
 * - The horizontal velocity of the entity is preserved during this operation.
 *
 * @example
 * // Move an entity vertically toward the mouse at a constant speed of 5.0f
 * units per second MoveTowardsMouseVertically(entity, 5.0f, 0.0f);
 *
 * @example
 * // Move an entity vertically toward the mouse within 2 seconds, regardless of
 * the distance MoveTowardsMouseVertically(entity, 0.0f, 2.0f);
 */
void PhysicsWorld::MoveTowardsMouseVertically(entt::entity entity, float speed,
  ...
  if (maxTime > 0.0f) {
    float distance =
        fabs(mousePosition.y - position.y); // Only vertical distance
    speed = distance / maxTime;
  }
 ...
 *
 * This method simulates the effect of torque on a rigid body by applying
 * balanced forces at offset points relative to the body's center of mass.
 *
 * @param entity The entity whose collider the torque is applied to.
 * @param torque The amount of torque to apply. Positive values apply
 * counterclockwise torque, and negative values apply clockwise torque.
 *
 * @note
 * - This implementation approximates torque using force vectors, as Chipmunk2D
 * does not provide a direct function for applying torque.
 * - Ensure the body's mass and moment of inertia are set correctly for accurate
 * torque effects.
 *
 * @example
...
void PhysicsWorld::ApplyTorque(entt::entity entity, float torque) {
  ...
  cpVect counterClockwiseForce =
      cpvmult(offset, torque); // Force in the opposite direction
 ...
 *
 * This function returns the mass of the body associated with the specified
 * entity's collider.
 *
 ...
 *
 * @note
 * - The mass is defined when the collider is created. It is used in
 * calculations for dynamics such as acceleration and force.
 * - Ensure that the collider has been initialized and properly set up in the
 * physics world before calling this function.
 *
 * @example
 ...
 *
 * This function updates the mass of the body associated with the specified
 * entity's collider. Changing the mass affects the physics calculations,
 * including acceleration and force interactions.
 *
 ...
 *
 * @note
 * - A mass of `0` or a negative value may cause unexpected behavior, depending
 * on the physics engine's implementation. Ensure the mass is a positive,
 * non-zero value unless intentionally setting it for a specific effect.
 * - Changing the mass of a dynamic body at runtime can impact the stability of
 ...
 *
 * Bullet mode emulates continuous collision detection (CCD) for high-speed
 * objects, reducing the chance of tunneling through other colliders. This
 * function adjusts the simulation parameters globally for improved accuracy.
 *
 ...
 *
 * @note
 * - Bullet mode is emulated by increasing simulation iterations and modifying
 * collision slop.
 * - Enabling bullet mode affects the global physics space and may increase
 * computational cost.
 *
 * @example
...
void PhysicsWorld::SetBullet(entt::entity entity, bool isBullet) {
  ...
  if (isBullet) {
    ...
    cpSpaceSetIterations(space,
                         20); // Adjust the number as needed for your simulation
    ...
    cpBodySetVelocityUpdateFunc(
        ...
        [](cpBody *body, cpVect gravity, cpFloat damping, cpFloat dt) {
          cpBodyUpdateVelocity(body, gravity, 1.0f,
                               dt); // No damping for precise velocity updates
        });

    // Optional: Reduce the collision slop globally for better accuracy
    cpSpaceSetCollisionSlop(
        space, 0.1); // Default is usually 0.5, smaller values reduce tunneling
  } else {
    // Restore default behavior
    cpSpaceSetIterations(space, 10); // Default iterations
    cpBodySetVelocityUpdateFunc(
        collider.body.get(),
        cpBodyUpdateVelocity);           // Reset to default velocity updates
 ...
 *
 * When fixed rotation is enabled, the body will not rotate regardless of
 * applied forces or collisions. Disabling fixed rotation restores the body's
 * ability to rotate.
 *
 * @param entity The entity whose rotation behavior is to be modified.
 * @param fixedRotation If true, prevents the body from rotating; otherwise,
 * allows it to rotate.
 *
 * @note
 * - Fixed rotation is achieved by setting the moment of inertia to infinity.
 * - Disabling fixed rotation restores the moment of inertia to a finite value,
 * which is calculated based on the shape and mass of the body.
 * - Ensure the shape dimensions are correctly provided when calculating the
 * moment of inertia.
 *
 * @example
...
void PhysicsWorld::SetFixedRotation(entt::entity entity, bool fixedRotation) {
  ...
  if (fixedRotation) {
    ...
    float moment =
        cpMomentForBox(mass, width, height); // Example for a box shape
    cpBodySetMoment(collider.body.get(), moment);
  }
}

/**
 * @brief Retrieves the vertices of a polygonal or chain shape associated with a
 * specified entity.
 *
 * This function extracts the vertices of a shape if the `ColliderShapeType`
 * indicates that it is either a `Polygon` or `Chain`. For chain shapes, the
 * vertices are closed into a loop by appending the first vertex to the end of
 * the vector.
 *
 * @param entity The entity whose shape vertices are to be retrieved.
 * @return std::vector<cpVect> A vector containing the vertices of the shape. If
 * the shape is not of type `Polygon` or `Chain`, the vector will be empty.
 *
 * @details
 * - This function relies on the `ColliderShapeType` enum within the
 * `ColliderComponent` to determine the type of the shape.
 * - For polygon shapes, the vertices are retrieved in order.
 * - For chain shapes, the vertices are retrieved in order, and the first vertex
 * is repeated at the end of the vector to close the loop.
 * - If the shape is not a `Polygon` or `Chain`, the function returns an empty
 * vector.
 * - The function assumes that the `collider.shape` pointer is valid and points
 * to a compatible shape in the Chipmunk2D physics system.
 *
 * @note
 * Ensure that the `ColliderShapeType` is correctly set when creating the
 * collider component. This function does not validate or infer the shape type
 ...
 *
 * The body type can be "static", "kinematic", or "dynamic". If the body type is
 * set to "dynamic", the body will be explicitly activated to ensure it
 * participates in the physics simulation.
 *
 * @param entity The entity whose body type is being modified.
 * @param bodyType A string specifying the desired body type: "static",
 ...
 * - **Kinematic**: Bodies that can move but are not affected by forces or
 * collisions.
 * - **Dynamic**: Bodies that are fully simulated, reacting to forces,
 * collisions, and impulses.
 * - When a body is set to dynamic, it is explicitly activated using
 * `cpBodyActivate` to ensure it does not remain in a sleeping state.
 *
 * @note
 * Ensure that the body type string is valid; otherwise, an exception will be
 * thrown.
 *
 * @example
...
void PhysicsWorld::SetBodyType(entt::entity entity,
  ...
  if (bodyType == "static") {
    ...
    cpBodyActivate(
        collider.body
            .get()); // Ensure the body is activated when set to dynamic
  } else {
...
entt::entity PhysicsWorld::PointQuery(float x, float y) {
  // Use nearest-point query with zero radius to pick the shape containing the
  // point.
...
/// @brief Build static geometry from a boolean tilemap exactly like the Obj‑C
/// demo.
...
void PhysicsWorld::CreateTilemapColliders(
  ...
  // 1) Sampling rect: from -0.5→w+0.5 so that samples hit tile centers
  // 0.5,1.5,…,w-0.5
...
void PhysicsWorld::OnGroupPostSolve(cpArbiter* arb) {
    // Extract the two shapes (and thus bodies) from the arbiter
    cpShape *shapeA, *shapeB;
    cpArbiterGetShapes(arb, &shapeA, &shapeB);
    cpBody  *bodyA = cpShapeGetBody(shapeA);
    cpBody  *bodyB = cpShapeGetBody(shapeB);

    // Ensure both bodies have UFNodes, then union their sets
    MakeNode(bodyA);
    MakeNode(bodyB);
    UnionBodies(bodyA, bodyB);

    // return cpTrue;  // allow the collision to proceed normally
}
...
UFNode& PhysicsWorld::MakeNode(cpBody* body) {
    ...
    if(it == _groupNodes.end()) {
        UFNode node;
        node.parent = &node;  // root of itself
        node.count  = 1;
...
UFNode& PhysicsWorld::FindNode(cpBody* body) {
    ...
    // Path compression: if not root, point directly to final root
    if(node.parent != &node) {
        // Cast parent pointer back to cpBody* by searching map (we store by cpBody*)
...
void PhysicsWorld::ProcessGroups() {
    // For each root node whose count ≥ threshold, call the user callback.
