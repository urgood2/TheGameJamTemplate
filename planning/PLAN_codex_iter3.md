# Cass Memory Update Plan v0 (Improved)
...
**Status:** Planning Complete - Ready for Execution (pending Phase 1 “environment verification” checklist)
...
> - Confirm the authoritative test entry file(s) (e.g., whether the engine loads `assets/scripts/test/binding_tests.lua` directly, or loads a scene script that `require`s it).
> - Decide a single canonical “run all tests” path early:
>   - interactive dev: run a scene that executes `binding_tests.lua`
