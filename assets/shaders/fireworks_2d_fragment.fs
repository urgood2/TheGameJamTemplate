...
vec4 boom(int c, vec2 suv, float t) {
    ...
    for (int i = 0; i < Praticle_num; i++) {
        ...
        float v2 = (1.0 / TAU) * exp(-((pow(x1 - ofx1, 2.0) + pow(x2 + ofx2, 2.0)) / (2.0 * ShneyMagnitude))) * s77;
...
void main()
{
    ...
    for (int j = 0; j < 6; j++) {
        ...
        vec2 duv = suv + Range * (1.0 - vec2(randomseed(float(j) * 37.0 + floor(tf)), randomseed(float(j) * 17.0 + floor(tf))) * 2.0);
        finalColor += boom(j, duv, mt);
    }
