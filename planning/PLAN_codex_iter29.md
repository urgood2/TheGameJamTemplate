# Game #4 “Bargain” — Engineering Plan (v17: deterministic sim, CI-reproducible, parallel workstreams)
...
### 0.2 Hard success criteria (must all be true)
- **Determinism:** same `(seed, script_id)` ⇒ same `run_state` + same `digest` (byte-for-byte).
- **Termination:** every run ends in `victory` or `death` (no softlocks), enforced by deterministic caps.
- **CI repro:** any failure prints **exactly one** single-line JSON object on stdout with enough info to replay locally.

### 0.3 Explicit non-goals (for MVP)
- Balance tuning beyond “downside is measurable”.
- Fancy UI polish, animations, particles, sound.
...
- stdout contains **exactly one** JSON repro line (see §8), and nothing else “JSON-shaped”.
...
## 2) CI structure & objective gates

### 2.1 CI jobs (fast + isolating)
- **Job 1: build** — `just build-debug-fast`
- **Job 2: bargain-headless** — `RUN_BARGAIN_TESTS=1 ./build-debug/...`
- (Optional later) **Job 3: smoke-run** — minimal runtime check (still headless), keeps UI out of CI.

### 2.2 Gates (must be cheap, objective, test-backed)
| Gate | What it proves | Exact check |
|---|---|---|
| A Determinism | reruns match | run each case twice; `digest` strings equal |
| B Termination | no softlocks | `run_state ∈ {"victory","death"}` for all scripts |
| C Deals catalog | stable IDs + complete set | exactly `21` deals; unique IDs; `7 sins × 3` coverage |
| D Downside enforcement | every deal has measurable downside | baseline vs applied flips ≥1 downside metric |
| E No sim nondeterminism | no hidden randomness/ordering drift | static scan + runtime invariants |
| F Repro fidelity | failures actionable | exactly one JSON repro line; schema-valid |

---

## 3) Architecture rules (enables parallel delivery)

### 3.1 Layer boundary (enforced)
...
### 3.2 Ownership to avoid merge collisions
Only two “hot” integration files should be frequently touched:
- `assets/scripts/core/main.lua`
- `assets/scripts/tests/run_bargain_tests.lua`

All other work lands in stream-owned directories (see §7).

---

## 4) Frozen interfaces (contract-tested; do not churn)

> Goal: freeze the smallest surface area that unblocks parallel work. Expand only with versioned changes.
...
Required fields (minimum viable determinism + replay):
- `world.seed`
...
- `world.stats` (downside metrics, see §10)
...
## 5) Determinism & termination enforcement
...
- `assets/scripts/tests/bargain/determinism_static_spec.lua`
  - bans nondeterministic APIs + bans `pairs()` in sim-critical scopes (with allowlist)
...
- `assets/scripts/tests/bargain/deals_downside_spec.lua`
  - 21 rows; each asserts ≥1 downside metric flips (see §10)
...
## 7) Parallel workstreams (low-collision ownership)

### 7.1 Streams & owned paths
...
### 7.2 Merge discipline (keeps streams unblocked)
...
3. Cross-stream needs go through interface extensions (versioned) or integrator wiring PRs.
...
### 8.2 Minimum payload (must replay deterministically)
...
### 8.3 Snapshot rules (only if enabled)
...
- repro schema validated on forced failure test
...
- CI gates A+B green across all cases (double-run + terminal)
...
### 10.2 Downside metrics (shared by tests; stored in `world.stats`)
...
## 12) Beads (bd) workflow integration

### 12.1 Milestone → beads rule
...
### 12.2 Per-bead operational rules
...
### 12.3 Suggested bead breakdown (parallel-friendly)
