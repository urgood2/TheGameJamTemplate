...
depth(j.contains("worldDepth") ? j["worldDepth"].get<int>() : 0)
{
    ...
    if (j["bgRelPath"].is_null()) {
        m_bg_image = null;
        m_bg_has_computed = false;
    } else {
        m_bg_image = BgImage();
        m_bg_image->path = FilePath(j["bgRelPath"].get<std::string>());

        // Read mode + pivot (always available when bgRelPath is set)
        if (!j["bgPos"].is_null())     m_bg_pos_mode = j["bgPos"].get<std::string>();
        if (!j["bgPivotX"].is_null())  m_bg_pivot.x = j["bgPivotX"].get<float>();
        if (!j["bgPivotY"].is_null())  m_bg_pivot.y = j["bgPivotY"].get<float>();

        // If editor provided resolved placement (__bgPos), use it
        if (!j["__bgPos"].is_null()) {
            m_bg_image->pos   = { j["__bgPos"]["topLeftPx"][0].get<int>(),
                                j["__bgPos"]["topLeftPx"][1].get<int>() };
            m_bg_image->scale = { j["__bgPos"]["scale"][0].get<float>(),
                                j["__bgPos"]["scale"][1].get<float>() };
            m_bg_image->crop  = { j["__bgPos"]["cropRect"][0].get<int>(),
                                j["__bgPos"]["cropRect"][1].get<int>(),
                                j["__bgPos"]["cropRect"][2].get<int>(),
                                j["__bgPos"]["cropRect"][3].get<int>() };
            m_bg_has_computed = true;
        } else {
            // No resolved placement: mark uncomputed; leave neutral values
            m_bg_image->pos   = {0, 0};
            m_bg_image->scale = {1.f, 1.f};
            m_bg_image->crop  = {0, 0, 0, 0}; // means "no crop"
            m_bg_has_computed = false;
        }
    }
