...
11/26/2025
  - Cast feed UI investigation: ensured module is required at gameplay scope and added a render node gated to action state so the feed draws every frame.
  - Added a persistent "CAST FEED READY" bootstrap message to validate visibility and exposed a lifetime override on feed items to keep it on-screen.
  - Fixed CastFeedUI crash by importing `core.z_orders` and letting infinite-lifetime items skip eviction.
  - Wired in PostHog telemetry end-to-end: engine client, Lua bindings, session tracking/lifecycle hooks, web logging, crash reporter signals, and README/BUILD telemetry docs; added unit coverage for Lua bindings and event bus/globals caches.
  - Exposed telemetry session ID to Lua and hooked gameplay to emit walk dust effects plus UI signals; web lifecycle hooks added for telemetry startup/shutdown.
  - Implemented tag-pattern matching/discovery system (per-cast tag analysis, tag-reactive Jokers, discovery journal/notifications) with docs and tests; integrated Cast Feed with discovery events and added Cast Feed test runners/render-layer fallbacks.
  - Overhauled projectile system (physics opt-in, manual gravity arcs, homing/steering fixes, world-bound refresh) and refreshed projectile tests.
  - Reworked card tooltip layout (label/value columns, padding, outlines) and tidied TODO prototype notes.
