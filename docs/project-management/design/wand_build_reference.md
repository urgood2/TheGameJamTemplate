...
- **Avatar (ascension):** Late-run macro mutation unlocked by a simple condition (kills/tag count/prayer uses); globally rewires a rule (e.g., multicast becomes loop; OnHit emits OnTick procs). -> could take the form of masks. don't make them move as heavily though. not sure why it does that- added mass?
