...
**Version:** v2 - Incorporates GPT Pro review revisions for reliability, maintainability, performance, and developer experience
...
7. **(New v2)** Capabilities file: `test_output/capabilities.json`
...
> - `register(name, category, testFn, opts)` where `opts` may include `tags`, `source_ref`, `timeout_frames` (if frame-driven), and `requires`.
...
- **Registration metadata**: `name`, `category`, `tags` (optional), `source_ref` (optional, e.g. C++ file or doc section), and `requires` (optional, capability list).
...
| Suite becomes slow as coverage grows | **(New v2)** Sharding support + enforced timeouts |
