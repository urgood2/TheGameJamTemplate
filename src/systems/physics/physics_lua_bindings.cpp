// Include full definitions first, before the header with forward declarations
...
// Now include the header with function declarations
#include "physics_lua_bindings.hpp"

namespace physics {

// ---- Lua <-> cpVect helpers -------------------------------------------------
static cpVect vec_from_lua(sol::table t) {
...
static std::vector<cpVect> vecarray_from_lua(sol::table arr) {
...
static sol::table vec_to_lua(sol::state_view L, const cpVect& v) {
...
void expose_physics_to_lua(sol::state& lua, EngineContext* ctx) {
...
void expose_steering_to_lua(sol::state& lua, EngineContext* ctx) {
    auto& rec = BindingRecorder::instance();
