...
auto initLineOfSight() -> void {
    ...
    los::MyVisibility::FuncGetDistance getDistance = [](int x, int y) {
        return util::getDistance(0, 0, x, y);
    };
...
namespace los
{
    ...
    MyVisibility::MyVisibility(MyVisibility::FuncBlocksLight blocksLight, MyVisibility::ActionSetVisible setVisible, MyVisibility::FuncGetDistance getDistance)
            : _blocksLight(blocksLight), _setVisible(setVisible), GetDistance(getDistance) {}

    auto MyVisibility::Compute(const LevelPoint &origin, int rangeLimit) -> void
    {
    ...
    auto MyVisibility::Compute(unsigned octant, LevelPoint origin, int rangeLimit, unsigned x, Slope top, Slope bottom) -> void
    {
        ...
        //    h
        for (; x <= static_cast<unsigned>(rangeLimit); x++) // (x <= (uint)rangeLimit) == (rangeLimit < 0 || x <= rangeLimit)
        {
            ...
            for (unsigned y = topY; (int)y >= (int)bottomY; y--) // use a signed comparison because y can wrap around when decremented
            {
                    ...
                    if (x != rangeLimit) // but don't bother adjusting them if this is the last column anyway
                    {
                        if (isOpaque)
                        {
                            ...
                            {                   // so adjust the bottom vector upward and continue processing it in the next column
                                ...
                                unsigned nx = x * 2, ny = y * 2 + 1; // top center by default
                        ...
                        else
                        {
                            if (wasOpaque > 0) // if we found a transition from opaque to clear, adjust the top vector downwards
                            {
                                ...
                                unsigned nx = x * 2, ny = y * 2 + 1; // the bottom of the opaque tile (oy*2-1) equals the top of this tile (y*2+1)
                                // NOTE: if you're using full symmetry and want more expansive walls (recommended), comment out the next line
    ...
    auto MyVisibility::BlocksLight(unsigned x, unsigned y, unsigned octant, const LevelPoint &origin) -> bool
    {
    ...
    auto MyVisibility::SetVisible(unsigned x, unsigned y, unsigned octant, const LevelPoint &origin) -> void
    {
        int nx = origin.X, ny = origin.Y;
        switch (octant)
        {
