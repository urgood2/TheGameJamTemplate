...
struct SteerableComponent {
    ...
    // behavior flags
    bool isSeeking=false, isFleeing=false, isPursuing=false, isEvading=false;
...
extern cpVect pointToWorld(const cpVect& local,
                           const cpVect& heading,
                           const cpVect& side,
                           const cpVect& position);
                           
extern cpVect pointToLocal(const cpVect& p, const cpVect& heading, const cpVect& side, const cpVect& pos);

extern cpVect vectorToWorld(const cpVect& v, const cpVect& heading, const cpVect& side);
...
// Lifecycle
//--------------------------------------------
extern void MakeSteerable(entt::registry& r,
                          entt::entity e,
                          float maxSpeed,
                          float maxForce,
                          float maxTurnRate = 2.0f * float(M_PI),
...
extern void SeekPoint(entt::registry& r, entt::entity e,
                      const cpVect& target,
                      float deceleration = 1.0f,
                      float weight = 1.0f);

// Separation (Chipmunk coords)
extern void Separate(entt::registry& r, entt::entity e,
                     float separationRadius,
                     const std::vector<entt::entity>& neighbors,
                     float weight = 1.0f);

// Wander (Chipmunk coords)
extern void Wander(entt::registry& r, entt::entity e,
                   float jitter,
                   float radius,
                   float distance,
                   float weight = 1.0f);
...
// FLee
extern void FleePoint(entt::registry& r, entt::entity e, const cpVect& threat, float panicDist, float weight=1.f);

extern void Pursuit(entt::registry& r, entt::entity e, entt::entity target, float weight=1.f);

extern void Evade(entt::registry& r, entt::entity e, entt::entity pursuer, float weight=1.f);

extern void Align(entt::registry& r, entt::entity e,
                  const std::vector<entt::entity>& neighbors,
                  float alignRadius, float weight=1.f);
...
extern void SetPath(entt::registry& r, entt::entity e,
                    ...
                    float arriveRadius = 16.f);

extern void PathFollow(entt::registry& r, entt::entity e,
                       float decel=1.0f, float weight=1.0f);

extern void ApplySteeringForce(entt::registry& r, entt::entity e, float f, float radians, float seconds);

extern void ApplySteeringImpulse(entt::registry& r, entt::entity e, float f, float radians, float seconds);
...
//--------------------------------------------
extern void SeekObject(entt::registry& r, entt::entity e, entt::entity target, float decel=1.f, float weight=1.f);
