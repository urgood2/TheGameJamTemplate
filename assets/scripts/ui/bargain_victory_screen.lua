-- assets/scripts/ui/bargain_victory_screen.lua
    ...
    return dsl.root({
        ...
        children = {
            dsl.vbox({
                ...
                children = {
                    dsl.text("Victory", { fontSize = UI(28), color = "gold", shadow = true }),
...
function VictoryScreen.set_context(world, on_restart)
    state.world = world
...
function VictoryScreen.show(world)
    if not dsl then
        return false
    end
...
function VictoryScreen.update(world, on_restart)
    ...
    if not ctx_world or not constants or ctx_world.run_state ~= constants.RUN_STATES.VICTORY then
