...
void propagateReleaseToGameObjects(input::InputState &inputState,
                                   entt::registry &registry);

void propagateDragToGameObjects(entt::registry &registry,
                                input::InputState &inputState);

void propagateClicksToGameObjects(entt::registry &registry,
                                  input::InputState &inputState);

void handleCursorHoverEvent(input::InputState &inputState,
                            entt::registry &registry);

void handleCursorReleasedEvent(input::InputState &inputState,
                               entt::registry &registry);

void handleCursorDownEvent(entt::registry &registry,
                           input::InputState &inputState);

void processRaylibLeftClick(input::InputState &inputState,
                            entt::registry &registry);
...
void ProcessControllerSnapToObject(input::InputState &inputState,
                                   entt::registry &registry);

void PropagateButtonAndKeyUpdates(input::InputState &inputState,
                                  entt::registry &registry, float dt);

void ProcessInputLocks(input::InputState &inputState, entt::registry &registry,
                       float dt);
...
// Cursor Management
auto SetCurrentCursorPosition(entt::registry &registry, InputState &state)
    -> void;
auto ModifyCurrentCursorContextLayer(entt::registry &registry,
                                     InputState &state, int delta) -> void;
auto SnapToNode(entt::registry &registry, InputState &state, entt::entity node,
                const Vector2 &transform = {0, 0}) -> void;
auto UpdateCursor(InputState &state, entt::registry &registry,
                  std::optional<Vector2> hardSetT = std::nullopt) -> void;

// Input Registry Management
auto DeleteInvalidEntitiesFromInputRegistry(InputState &state,
                                            entt::registry &registry) -> void;
auto AddNodeToInputRegistry(entt::registry &registry, InputState &state,
                            entt::entity node, const GamepadButton button)
    -> void;
...
auto ProcessTextInput(entt::registry &registry, entt::entity entity,
                      KeyboardKey key, bool shift, bool capsLock) -> void;
...
void KeyboardKeyPressUpdate(entt::registry &registry, InputState &state,
                            KeyboardKey key, float dt);
...
void MarkEntitiesCollidingWithCursor(entt::registry &registry,
                                     InputState &state,
                                     const Vector2 &cursor_trans);
...
void ProcessLeftMouseButtonPress(entt::registry &registry, InputState &state,
                                 float x = -1, float y = -1);
...
bool CaptureFocusedInput(entt::registry &registry, InputState &state,
                         const std::string inputType, GamepadButton button,
                         float dt);
...
auto DispatchRaw(InputState &s, InputDeviceInputCategory dev, int code,
                 bool down, float value = 0.f) -> void;
// Tick held timers; call once per frame before DecayActions
auto TickActionHolds(InputState &s, float dt) -> void;
auto bind_action(InputState &s, const std::string &action,
                 const ActionBinding &b) -> void;
...
auto start_rebind(InputState &s, const std::string &action,
                  std::function<void(bool, ActionBinding)> cb) -> void;
} // namespace input
