...
#define MAXACTIONS 64
	...
	/*
	 * Note: Updated the actionplanner_t structure to use 'char*' instead of 'const char*' for atom and action names.
	 * This allows us to store copies of strings (non-literal strings) and manage their memory properly.
	 * Ensure to update this in your 'goap.h' header file as well.
	 *
	 * typedef struct
	 * {
	 *     char* atm_names[ MAXATOMS ];   //!< Names associated with all world state atoms.
	 *     int numatoms;                  //!< Number of world state atoms.
	 *
	 *     char* act_names[ MAXACTIONS ]; //!< Names of all actions in repertoire.
	 ...
	 *     int numactions;                    //!< The number of actions in our repertoire.
	 *
	 * } actionplanner_t;
	 */
	typedef struct
