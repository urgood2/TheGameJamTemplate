...
// * 3. Add the method binding in the
// initLuaStateWithAllObjectAndFunctionBindings method in this file
// * 4. Make sure the above method is called to initialize the lua state you
// plan to use
// * 5. Profit
...
/**
 * ------------------------------------------------------
 * Master lua state initialization function - the master state contains all the
 * bindings for running the ai system from the lua side
 * ------------------------------------------------------
 */
auto initLuaMasterState(sol::state &stateToInit,
                        const std::vector<std::string> scriptFilesToRead,
                        EngineContext *ctx) -> void {

  auto &rec = BindingRecorder::instance();

  // basic lua state initialization
  stateToInit.open_libraries(sol::lib::base, sol::lib::package, sol::lib::table,
                             sol::lib::coroutine, sol::lib::os,
                             sol::lib::string, sol::lib::math, sol::lib::debug,
  ...
  std::string lua_path_cmd = "package.path = '" + base1 + "?.lua;" + base1 +
                             "?/?.lua;" + base1 + "?/?/?.lua;" + base2 +
                             "?.lua;" + base3 + "?.lua;" + base5 + "?.lua;" +
                             base4 + "?.lua;scripts/?/?.lua" +
                             "' .. package.path"; // <- correctly attached

  // set the lua path to include the scripts directory
  stateToInit.script(lua_path_cmd, [](lua_State *,
                                      sol::protected_function_result pfr) {
    // pfr will contain things that went wrong, for either loading or executing
    // the script Can throw your own custom error You can also just return it,
    // and let the call-site handle the error if necessary.
  ...
  rec.record_free_function({}, {"hardReset", "---@return nil",
  ...
  //---------------------------------------------------------
  stateToInit.new_enum("ActionResult", "SUCCESS", Action::Result::SUCCESS,
                       "FAILURE", Action::Result::FAILURE, "RUNNING",
  ...
  //---------------------------------------------------------
  // methods from shader_pipeline.cpp. These can be called from lua✅ TODO: type
  // registration not done properly
  //---------------------------------------------------------
  ...
  stateToInit.set_function("setCurrentWorldStateValue",
                           setCurrentWorldStateValue);
  stateToInit.set_function("getCurrentWorldStateValue",
                           getCurrentWorldStateValue);
  ...
  rec.record_free_function(
      {}, {"setBlackboardFloat",
           "---@param entity Entity\n---@param key string\n---@param value "
           "number\n---@return nil",
           "Sets a float value on an entity's blackboard.", true, false});
  rec.record_free_function(
      {}, {"getBlackboardFloat",
           "---@param entity Entity\n---@param key string\n---@return number",
  ...
  rec.record_free_function(
      {}, {"setBlackboardVector2",
           "---@param entity Entity\n---@param key string\n---@param value "
           "Vector2\n---@return nil",
           "Sets a Vector2 value on an entity's blackboard.", true, false});
  rec.record_free_function(
      {}, {"getBlackboardVector2",
           "---@param entity Entity\n---@param key string\n---@return Vector2",
           "Gets a Vector2 value from an entity's blackboard.", true, false});
  ...
  rec.record_free_function(
      {}, {"setBlackboardBool",
           "---@param entity Entity\n---@param key string\n---@param value "
           "boolean\n---@return nil",
           "Sets a boolean value on an entity's blackboard.", true, false});
  rec.record_free_function(
      {}, {"getBlackboardBool",
           "---@param entity Entity\n---@param key string\n---@return boolean",
           "Gets a boolean value from an entity's blackboard.", true, false});

  stateToInit.set_function(
      "blackboardContains", [](entt::entity entity, const std::string &key) {
        auto &blackboard =
            globals::getRegistry().get<GOAPComponent>(entity).blackboard;
        return blackboard.contains(key);
      });
  rec.record_free_function(
      {}, {"blackboardContains",
           "---@param entity Entity\n---@param key string\n---@return boolean",
           "Checks if the blackboard contains a specific key.", true, false});
  ...
  rec.record_free_function(
      {}, {"setBlackboardInt",
           "---@param entity Entity\n---@param key string\n---@param value "
           "integer\n---@return nil",
           "Sets an integer value on an entity's blackboard.", true, false});
  rec.record_free_function(
      {}, {"getBlackboardInt",
           "---@param entity Entity\n---@param key string\n---@return integer",
           "Gets an integer value from an entity's blackboard.", true, false});
  ...
  rec.record_free_function(
      {}, {"setBlackboardString",
           "---@param entity Entity\n---@param key string\n---@param value "
           "string\n---@return nil",
           "Sets a string value on an entity's blackboard.", true, false});
  rec.record_free_function(
      {}, {"getBlackboardString",
           "---@param entity Entity\n---@param key string\n---@return string",
           "Gets a string value from an entity's blackboard.", true, false});
  ...
  stateToInit.set_function("add_fullscreen_shader",
                           game::add_fullscreen_shader);
  stateToInit.set_function("remove_fullscreen_shader",
  ...
  // quad tree
  // ------------------------------------------------------
  game::luaqt::bind_quadtrees_lua(stateToInit, globals::quadtreeWorld,
                                  globals::quadtreeUI);
  ...
  rec.dump_lua_defs(
      util::getRawAssetPathNoUUID("scripts/chugget_code_definitions.lua"));
}
...
auto exposeGlobalsToLua(sol::state &lua) -> void {
  ...
  typedef struct Camera2D {
      ...
      float zoom;             // Camera zoom (scaling), should be 1.0f by
  default } Camera2D;
  */
  ...
  // isntead
  static Camera2D
      camera2D{}; // create a static camera2D object to use as a global
  ...
  rec.record_property("", {"globalShaderUniforms", "nil",
                           "global ShaderUniformComponent object, used to set "
                           "shader uniforms globally."});

  // Sprite frame + atlas helper for Lua (used by shader uniform setup)
  rec.bind_function(
      lua, {}, "getSpriteFrameTextureInfo",
      [&lua](const std::string &identifier) -> sol::optional<sol::table> {
        ...
        if (!atlasTex) {
          SPDLOG_ERROR(
              "getSpriteFrameTextureInfo: atlas '{}' not found for '{}'",
              frame.atlasUUID, identifier);
          return sol::nullopt;
        }
        ...
        t["frame"] = sv.create_table_with(
            "x", frame.frame.x, "y", frame.frame.y, "width", frame.frame.width,
            "height", frame.frame.height);
        t["gridRect"] = Vector4{frame.frame.x, frame.frame.y, frame.frame.width,
                                frame.frame.height};
        t["imageSize"] = Vector2{static_cast<float>(atlasTex->width),
                                 static_cast<float>(atlasTex->height)};
        return t;
      },
      R"lua(
  ...
  rec.bind_function(
      ...
      "Loads a palette texture from disk and uploads it to the shader's "
      "'palette' uniform with point filtering.");
...
auto luaDebugLogWrapper(entt::entity entity, std::string message) -> void {
  spdlog::default_logger()->debug("[LUA] {}: {}", static_cast<int>(entity),
                                  message);
}

auto luaErrorLogWrapper(entt::entity entity, std::string message) -> void {
  spdlog::default_logger()->error("[LUA] {}: {}", static_cast<int>(entity),
                                  message);
}
...
// TODO: add component-level access for the most commonly used components,
// probably on a need-to-use basis
...
bool isKeyPressed(const std::string &key) {
  ...
  auto keyEnumOpt = magic_enum::enum_cast<KeyboardKey>(
      key, magic_enum::case_insensitive); // ignore case
  if (!keyEnumOpt.has_value()) {
    SPDLOG_ERROR("Key {} not found in enum",
                 key); // Log an error if the key is not found
    return false;
  }

  // Check if the key is pressed using IsKeyPressed (assuming it accepts
  // KeyboardKey)
...
auto setCurrentWorldStateValue(entt::entity entity, std::string key, bool value)
    -> void {
  auto &goapComponent = globals::getRegistry().get<GOAPComponent>(entity);
  goap_worldstate_set(&goapComponent.ap, &goapComponent.current_state,
                      key.c_str(), value);
  SPDLOG_DEBUG("{}: Setting current world state \"{}\" to {}",
               static_cast<int>(entity), key, value);
}

auto getCurrentWorldStateValue(entt::entity entity, std::string key) -> bool {
  auto &goapComponent = globals::getRegistry().get<GOAPComponent>(entity);
  bool value = false;
  bool successful = ai_system::goap_worldstate_get(
      &goapComponent.ap, goapComponent.current_state, key.c_str(), &value);
  if (!successful) {
    SPDLOG_ERROR("{}: Could not get current world state \"{}\": does not "
                 "exist. Defaulting to false",
                 static_cast<int>(entity), key);
  }
  return value;
}
...
auto setGoalWorldStateValue(entt::entity entity, std::string key, bool value)
    -> void {
  auto &goapComponent = globals::getRegistry().get<GOAPComponent>(entity);
  goap_worldstate_set(&goapComponent.ap, &goapComponent.goal, key.c_str(),
                      value);
  SPDLOG_DEBUG("{}: Setting goal world state \"{}\" to {}",
               static_cast<int>(entity), key, value);
}
...
auto getGoalWorldStateValue(entt::entity entity, std::string key) -> bool {
  ...
  bool successful = ai_system::goap_worldstate_get(
      &goapComponent.ap, goapComponent.current_state, key.c_str(), &value);
  if (!successful) {
    SPDLOG_ERROR("{}: Could not get goal world state \"{}\": does not exist. "
                 "Defaulting to false",
                 static_cast<int>(entity), key);
  }
  return value;
...
auto setBlackboardVector2(entt::entity entity, std::string key,
  ...
  // SPDLOG_DEBUG("{}: Setting blackboard vector2 \"{}\" to ({}, {})",
...
auto getBlackboardVector2(entt::entity entity, std::string key)
  ...
  // SPDLOG_DEBUG("{}: Getting blackboard vector2 \"{}\": ({}, {})",
  // static_cast<int>(entity), key, value.x, value.y);
  return value;
}

auto setBlackboardFloat(entt::entity entity, std::string key, float valueToSet)
    -> void {
  if (!globals::getRegistry().valid(entity) || entity == entt::null) {
    SPDLOG_ERROR("Entity {} is not valid, cannot set blackboard float",
                 static_cast<int>(entity));
    return;
  }
  auto &blackboard =
  ...
  // SPDLOG_DEBUG("{}: Setting blackboard float \"{}\" to {}",
  // static_cast<int>(entity), key, valueToSet);
}

auto getBlackboardFloat(entt::entity entity, std::string key)
    -> std::optional<float> {
  if (!globals::getRegistry().valid(entity) || entity == entt::null) {
    SPDLOG_ERROR("Entity {} is not valid, cannot get blackboard float",
                 static_cast<int>(entity));
  ...
  // SPDLOG_DEBUG("{}: Getting blackboard float \"{}\": {}",
  // static_cast<int>(entity), key, value);
  return value;
}

auto setBlackboardBool(entt::entity entity, std::string key, bool valueToSet)
    -> void {
  if (!globals::getRegistry().valid(entity) || entity == entt::null) {
    SPDLOG_ERROR("Entity {} is not valid, cannot set blackboard bool",
                 static_cast<int>(entity));
    return;
  }
  auto &blackboard =
      globals::getRegistry().get<GOAPComponent>(entity).blackboard;
  blackboard.set(key, valueToSet);
  // SPDLOG_DEBUG("{}: Setting blackboard bool \"{}\" to {}",
  // static_cast<int>(entity), key, valueToSet);
}

auto getBlackboardBool(entt::entity entity, std::string key)
    -> std::optional<bool> {
  if (!globals::getRegistry().valid(entity) || entity == entt::null) {
    SPDLOG_ERROR("Entity {} is not valid, cannot get blackboard bool",
                 static_cast<int>(entity));
  ...
  // SPDLOG_DEBUG("{}: Getting blackboard bool \"{}\": {}",
  // static_cast<int>(entity), key, value);
  return value;
}

auto setBlackboardInt(entt::entity entity, std::string key, int valueToSet)
    -> void {
  if (!globals::getRegistry().valid(entity) || entity == entt::null) {
    SPDLOG_ERROR("Entity {} is not valid, cannot set blackboard int",
                 static_cast<int>(entity));
    return;
  }
  auto &blackboard =
      globals::getRegistry().get<GOAPComponent>(entity).blackboard;
  blackboard.set(key, valueToSet);
  // SPDLOG_DEBUG("{}: Setting blackboard int \"{}\" to {}",
  // static_cast<int>(entity), key, valueToSet);
}

auto getBlackboardInt(entt::entity entity, std::string key)
    -> std::optional<int> {
  if (!globals::getRegistry().valid(entity) || entity == entt::null) {
    SPDLOG_ERROR("Entity {} is not valid, cannot get blackboard int",
                 static_cast<int>(entity));
  ...
  // SPDLOG_DEBUG("{}: Getting blackboard int \"{}\": {}",
  // static_cast<int>(entity), key, value);
  return value;
}

auto setBlackboardString(entt::entity entity, std::string key,
                         std::string valueToSet) -> void {
  if (!globals::getRegistry().valid(entity) || entity == entt::null) {
    SPDLOG_ERROR("Entity {} is not valid, cannot set blackboard string",
                 static_cast<int>(entity));
    return;
  }
  auto &blackboard =
      globals::getRegistry().get<GOAPComponent>(entity).blackboard;
  blackboard.set(key, valueToSet);
  // SPDLOG_DEBUG("{}: Setting blackboard string \"{}\" to {}",
  // static_cast<int>(entity), key, valueToSet);
}

auto getBlackboardString(entt::entity entity, std::string key)
    -> std::optional<std::string> {
  if (!globals::getRegistry().valid(entity) || entity == entt::null) {
    SPDLOG_ERROR("Entity {} is not valid, cannot get blackboard string",
                 static_cast<int>(entity));
  ...
  // SPDLOG_DEBUG("{}: Getting blackboard string \"{}\": {}",
  // static_cast<int>(entity), key, value);
  return value;
}
...
void dump_lua_globals(sol::state &lua, std::string const &out_path) {
  ...
  // (if you want to preserve your old hook, you could still munge `capture`
  // here)
