# Master Plan: End-to-End (E2E) Game Testing System — v3

This document integrates GPT Pro's v2.1 revisions into the v2 plan, adding versioned schemas, external supervision, baseline safety mechanisms, performance budgets, flake diagnostics, and sandboxing improvements.
...
- Run tests via:
  ...
  - (CI recommended): `./tools/e2e_supervisor -- <game args...>` for hardened timeout/crash handling
- Tests can:
  - **drive input**
...
8. `determinism_guard`: runtime tripwires for time/RNG/fs/async violations.
...
4. Reporters: JSON + JUnit (TAP optional).
   - JSON authoritative includes environment, resolved roots, per-test durations, artifacts, `failure_kind`.
...
5. `baseline_manager`: resolve baseline dir by platform/baseline_key/resolution; implement update-baselines with staging; merge `.meta.json`.
...
9. `timeline_writer`: merged `timeline.jsonl` with inputs/logs/screenshots/hashes.
...
1. Workflow `.github/workflows/e2e-tests.yml`:
   - build + cache
   - run headless tests via supervisor
...
- Phase 0 interface freeze: flags, `test_harness` contract, output paths, report schema, determinism contract (incl FP/env/fs), JSON schemas.
...
- Also: `test_runtime`, `path_sandbox`, `artifact_store`, `lua_sandbox`, `determinism_guard`.
...
- Phase 4 + Phase 6: state query, screenshot diff, baseline manager, log capture, GitHub Actions, summary script, supervisor.
...
1. **M0 (Contracts frozen):** flags + `test_harness` API + paths + report schema agreed and stubbed; determinism contract documented (incl FP/env/fs); JSON schemas committed and validated.
...
6. **M5 (Coverage):** 3 stable example tests + docs for adding tests/updating baselines; forensics bundle on failures; timeline.jsonl written.
7. **M6 (Hardening):** determinism audit tripwire + per-test timeouts + skip/xfail/quarantine fully wired; optional isolate-tests mode; Lua sandbox enforced; flake triage automation.
8. **M7 (Polish):** perf budgets optional; seeded shuffle mode; full HTML gallery with all artifact types.
...
| File | Description |
|------|-------------|
| `run_manifest.json` | CLI args, seed, platform, renderer, resolution, git SHA, fingerprints, ordering hash |
...
## Appendix F: Changelog from v2

### Major Additions (v3)
1. **Section 0.2 Report + Manifest Contracts:** Versioned schemas, stable failure taxonomy, fingerprints
2. **Section 1.1.1 Tools:** External e2e_supervisor for CI hardening
3. **Lua sandboxing:** `lua_sandbox.{hpp,cpp}`, `--lua-sandbox` flag, section 3.9
4. **Determinism tripwires:** `determinism_guard.{hpp,cpp}`, `--determinism-violation` flag
5. **Baseline staging:** `--baseline-write-mode`, `--baseline-staging-dir`, `--baseline-approve-token`
6. **HTML gallery:** `index.html` artifact for visual debugging
7. **Timeline log:** `timeline.jsonl` merged event stream
8. **Seeded shuffle:** `--shuffle-tests`, `--shuffle-seed` for order-dependence detection
9. **Performance hooks:** `--perf-mode`, `--perf-budget`, section 3.8 API
10. **Flake diagnostics:** `--auto-audit-on-flake`, `--flake-artifacts`
11. **Input state API:** `input_state()`, `screen_to_ui()`, `ui_to_screen()` in section 3.2.1
12. **Snapshot contract:** Explicit semantics for `snapshot_create`/`snapshot_restore` with error codes
13. **Fixtures directory:** `assets/scripts/tests/fixtures/` for deterministic test data
14. **CI linting:** Appendix E for determinism API grep checks
15. **Milestone M7:** Added for polish features

### Structural Changes
1. Supervisor section added under CLI Contract
2. Isolation requirements elevated from "prefer" to "MUST be available"
3. Milestones M2/M3 updated with isolation/snapshot requirements
4. New Appendix D (Report Schema) and Appendix E (CI Linting)
5. Phase 4 expanded with `artifact_index`, `timeline_writer`, `perf_tracker`
6. Phase 6 includes supervisor integration
