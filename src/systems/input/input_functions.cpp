...
namespace input {
    ...
    auto PollInput(entt::registry &registry, InputState &inputState, float dt) -> void
    {
        ...
        if (mouseDetectDownFirstFrameLeft) { // this should only register first time the button is held down
            ReconfigureInputDeviceInfo(inputState, InputDeviceInputCategory::MOUSE);
        ...
        if (mosueLeftDownCurrentFrame == false && mouseLeftDownLastFrame == true) { // release only for left button
            ReconfigureInputDeviceInfo(inputState, InputDeviceInputCategory::MOUSE);
            Vector2 mousePos = GetMousePosition();
        ...
        // poll gamepad
        if (IsGamepadAvailable(0)) { // just the one gamepad
    ...
    auto Update(entt::registry &registry, InputState &inputState, float dt) -> void {
        ...
        if (inputState.L_cursor_queue) { // cursor click has been queued by raylib
        ...
        // TODO: clarify click and release, maybe down and up is better
        ...
        // is the cursor down, but not handled?
        if (registry.valid(inputState.cursor_down_target) && inputState.cursor_down_handled == false) { //TODO: so this is false if cursor is down and hasn't been handled?
        ...
        // cursor is hovering over something
        ...
        // propogate input updates to game objects
        
        // the node clicked upon
        ...
        // handle current drag target
        // SPDLOG_DEBUG("Cursor dragging target: {}", static_cast<int>(inputState.cursor_dragging_target));
        if (registry.valid(inputState.cursor_dragging_target)) {
        ...
        //TODO: clarify released in docs. Released is when something was dragged, then released.
        // handle mouse release target
        // release not handled, previous drag is valid, meaning it was dragging something.
    ...
    std::string DeduceConsoleFromGamepad(int gamepadIndex) {
        if (!IsGamepadAvailable(gamepadIndex)) return "No Gamepad";
    ...
    auto UpdateGamepadAxisInput(InputState &state, entt::registry &registry, float dt) -> InputDeviceInputCategory
    {
        ...
        if (state.hid.controller_enabled == false) axisInterpretation = InputDeviceInputCategory::MOUSE;
    ...
    char GetCharacterFromKey(KeyboardKey key, bool caps) {
        static std::unordered_map<KeyboardKey, std::pair<char, char>> keyMap = {
            ...
            { KEY_Y, {'y', 'Y'} }, { KEY_Z, {'z', 'Z'} }, { KEY_ZERO, {'0', ')'} }, { KEY_ONE, {'1', '!'} },
            { KEY_TWO, {'2', '@'} }, { KEY_THREE, {'3', '#'} }, { KEY_FOUR, {'4', '$'} }, { KEY_FIVE, {'5', '%'} },
            { KEY_SIX, {'6', '^'} }, { KEY_SEVEN, {'7', '&'} }, { KEY_EIGHT, {'8', '*'} }, { KEY_NINE, {'9', '('} },
    ...
    void ProcessTextInput(entt::registry& registry, entt::entity entity, KeyboardKey key, bool shift, bool capsLock) {
        ...
        // **Enter: Finish input and execute callback**
            ...
            if (textInput.callback) textInput.callback();
    ...
    void ProcessLeftMouseButtonPress(entt::registry &registry, InputState& state, float x, float y) {
        ...
        if (x < 0.0f) x = state.cursor_position.x;
        if (y < 0.0f) y = state.cursor_position.y;
        ...
        if (temporaryListOfPotentiallyFocusableNodes.empty() == false) {
            if (dir) {
                ...
                if ((dir == "L" || dir == "R") && registry.valid(state.cursor_focused_target) && focusedTargetisGameEntityWithFocusing) { 
                    ...
                    for (auto entity : temporaryListOfPotentiallyFocusableNodes) { //REVIEW: focusables are checked, then added to focus_list for processing
                        if (entity.node == state.current_designated_hover_target || entity.node == state.cursor_focused_target) continue;
                        ...
                        if (uiConfig.focusArgs && uiConfig.focusArgs->nav) {
                            if (uiConfig.focusArgs->nav == "wide") {
                                if (focus_vec.y > 0.1f && dir == "D") eligible = true;
                                else if (focus_vec.y < -0.1f && dir == "U") eligible = true;
                                else if (std::abs(focus_vec.y) < targetNodeTransform.getActualH() / 2) eligible = true;
                            } else if (uiConfig.focusArgs->nav == "tall") {
                                if (focus_vec.x > 0.1f && dir == "R") eligible = true;
                                else if (focus_vec.x < -0.1f && dir == "L") eligible = true;
                                else if (std::abs(focus_vec.x) < targetNodeTransform.getActualW() / 2) eligible = true;
                            }
                        }
                        // If no nav type exists, determine the dominant movement direction:
                        else if (std::abs(focus_vec.x) > std::abs(focus_vec.y)) {
                            if (focus_vec.x > 0 && dir == "R") eligible = true;
                            else if (focus_vec.x < 0 && dir == "L") eligible = true;
                        } else {
                            if (focus_vec.y > 0 && dir == "D") eligible = true;
                            else if (focus_vec.y < 0 && dir == "U") eligible = true;
                        }
            ...
            if (focusedNodeUIConfig.focusArgs) {
                ...
                if (focusedNodeUIConfig.focusArgs->type.value() == "tab" && inputType == "press") {
                    ...
                    for (size_t i = 0; i < choices.size(); ++i) {
                        ...
                        if (choiceNodeUIConfig.chosen) { // Found selected tab
                            size_t nextIndex;
