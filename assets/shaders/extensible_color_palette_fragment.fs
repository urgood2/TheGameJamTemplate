...
void main()
{
    vec3 tex_uv = vec3(fragTexCoord, 0.0);
    vec3 color_input;
    float alpha_input;

    // Get the colors from the image at specified location
    vec4 _tex_read = textureLod(texture0, tex_uv.xy, 0.0);
    color_input = _tex_read.rgb;
    alpha_input = _tex_read.a;
    ...
    // Blend between your two color maps
    finalColor = vec4(c_lerp(colorP1, colorP0, lerper), alpha_input);
