# Master Plan: End-to-End (E2E) Game Testing System — v2

This document integrates GPT Pro's architecture hardening, determinism upgrades, failure forensics improvements, and CI survivability enhancements into the original plan.
...
## Appendix D: Changelog from v1

### Major Additions
1. **Determinism Contract Rules 6-9:** FP behavior pinning, process environment pinning, filesystem ordering, GPU sync points
2. **Per-test timeouts:** `--default-test-timeout-frames` for frame-based per-test watchdogs
3. **Skip/XFail/Quarantine:** Test lifecycle states for CI hygiene
4. **Baseline keying:** `--baseline-key` for renderer/colorspace-specific baselines
5. **Determinism audit mode:** Multi-run divergence detection with `--determinism-audit`
6. **Input record/replay:** `--record-input` / `--replay-input` for debugging
7. **Process isolation:** `--isolate-tests` option for crash containment
8. **Structured logs:** `logs.jsonl` with frame/level/category/message format
9. **Repro scripts:** Auto-generated `repro.sh` and `repro.ps1` in forensics
10. **Capabilities API:** `test_harness.capabilities`, `test_harness.require()`
11. **Frame hashing:** `test_harness.frame_hash()` for determinism verification

### Structural Changes
1. Non-Goals scoped as "(v1)" for clarity
2. New `determinism_audit.{hpp,cpp}` in C++ structure
3. Baseline path includes `<baseline_key>` level
4. Expanded forensics bundle specification
5. Milestone M6 added for hardening features
