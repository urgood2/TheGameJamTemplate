...
void main() {
    ...
    vec2 localUV = getSpriteUV(fragTexCoord);      // 0..1 in sprite space
    vec2 centered = localUV - 0.5;
    float dist = length(centered);
    vec2 dir = dist > 0.0001 ? centered / dist : vec2(0.0);

    const float SPREAD = 0.85;        // How far pixels are pushed out
    const float WOBBLE = 0.035;       // Secondary distortion strength
    const float FADE_START = 0.45;    // When alpha starts dropping
    ...
    // --- Material sheen / foil look (reuse original effect) ---
    ...
    float edgeDistance = length(warpedLocal - clampedLocal); // non-zero when pushed outside
